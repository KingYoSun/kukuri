...
impl SignatureService for DefaultSignatureService {
    async fn sign_event(&self, event: &mut Event, private_key: &str) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        // イベントIDを計算
        let tags: Vec<nostr_sdk::Tag> = event.tags.clone().into_iter().map(|t| {
        ...
        let mut event_builder = nostr_sdk::EventBuilder::new(
...
mod tests {
    ...
    async fn test_sign_and_verify_message() {
        ...
        let is_valid = service.verify_message(message, &signature, &public_key).await.unwrap();
        assert!(is_valid);
    }
}