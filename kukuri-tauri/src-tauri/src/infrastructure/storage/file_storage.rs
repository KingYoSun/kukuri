...
pub trait FileStorage: Send + Sync {
    async fn save_file(&self, path: &PathBuf, data: &[u8]) -> Result<(), Box<dyn std::error::Error>>;
    ...
    async fn list_files(&self, directory: &PathBuf) -> Result<Vec<PathBuf>, Box<dyn std::error::Error>>;
