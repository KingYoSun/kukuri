...
pub async fn initialize_nostr(state: State<'_, AppState>) -> Result<(), String> {
    ...
    event_manager.initialize_with_key_manager(key_manager)
        .await
...
pub async fn add_relay(url: String, state: State<'_, AppState>) -> Result<(), String> {
    state.event_manager.add_relay(&url)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn publish_text_note(content: String, state: State<'_, AppState>) -> Result<String, String> {
    let event_id = state.event_manager.publish_text_note(&content)
        .await
        .map_err(|e| e.to_string())?;
...
pub async fn publish_topic_post(
    ...
    let event_id = state.event_manager.publish_topic_post(&topic_id, &content, reply_event_id)
        .await
        .map_err(|e| e.to_string())?;
...
pub async fn send_reaction(
    ...
    let reaction_event_id = state.event_manager.send_reaction(&target_event_id, &reaction)
        .await
        .map_err(|e| e.to_string())?;
...
pub async fn update_nostr_metadata(
    ...
    let event_id = state.event_manager.update_metadata(nostr_metadata)
        .await
        .map_err(|e| e.to_string())?;
...
/// トピックをサブスクライブ
#[tauri::command]
pub async fn subscribe_to_topic(topic_id: String, state: State<'_, AppState>) -> Result<(), String> {
    state.event_manager.subscribe_to_topic(&topic_id)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn subscribe_to_user(pubkey: String, state: State<'_, AppState>) -> Result<(), String> {
    ...
    state.event_manager.subscribe_to_user(public_key)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn delete_events(
    ...
    let ids: Result<Vec<EventId>, _> = event_ids.iter()
        .map(|id| EventId::from_hex(id))
        .collect();
    
    let ids = ids.map_err(|e| e.to_string())?;
    
    let event = publisher.create_deletion(ids, reason.as_deref())
        .map_err(|e| e.to_string())?;
    
    let client_manager = event_manager.client_manager.read().await;
    let event_id = client_manager.publish_event(event)
        .await
        .map_err(|e| e.to_string())?;
...
pub async fn disconnect_nostr(state: State<'_, AppState>) -> Result<(), String> {
    state.event_manager.disconnect()
        .await
        .map_err(|e| e.to_string())
}
