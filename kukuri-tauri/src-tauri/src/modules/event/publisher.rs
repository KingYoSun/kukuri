...
mod tests {
    ...
    fn test_create_text_note_with_tags() {
        ...
        let event = publisher.create_text_note("Hello with tags!", tags.clone()).unwrap();
    ...
    fn test_create_repost() {
        ...
        let event_id = EventId::from_slice(&[1; 32]).unwrap();
        let event = publisher.create_repost(&event_id, Some("wss://relay.test".to_string())).unwrap();
    ...
    fn test_create_custom_event() {
        ...
        let event = publisher.create_custom_event(custom_kind, content, tags).unwrap();
    ...
    fn test_create_deletion() {
        ...
        let event = publisher.create_deletion(event_ids.clone(), Some("Spam")).unwrap();
    ...
    fn test_create_topic_post() {
        ...
        assert!(tags.iter().any(|t| t.kind().to_string() == "topic" && t.content().is_some()));
    }
    ...
    fn test_create_topic_post_with_reply() {
        ...
        let event = publisher.create_topic_post("nostr", "Reply to thread", Some(reply_to)).unwrap();
        ...
        assert!(tags.iter().any(|t| t.kind().to_string() == "reply" && t.content().is_some()));
    }
    ...
    fn test_no_keys_error() {
        ...
        assert!(publisher.create_reaction(&EventId::from_slice(&[1; 32]).unwrap(), "+").is_err());
        assert!(publisher.create_repost(&EventId::from_slice(&[1; 32]).unwrap(), None).is_err());
        assert!(publisher.create_custom_event(Kind::Custom(1), "test", vec![]).is_err());
        assert!(publisher.create_deletion(vec![], None).is_err());
        assert!(publisher.create_topic_post("topic", "content", None).is_err());
    }
    ...
    fn test_event_signature_verification() {
        ...
        let events = vec![
            ...
            publisher.create_metadata(Metadata::new().name("test")).unwrap(),
            publisher.create_reaction(&EventId::from_slice(&[1; 32]).unwrap(), "+").unwrap(),
            publisher.create_topic_post("test", "content", None).unwrap(),
        ];
        
        for event in events {
            assert!(event.verify().is_ok(), "Event signature verification failed");
        }
    }
}
