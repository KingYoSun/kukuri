...
mod tests {
    ...
    impl TestSecureStorage {
        ...
        fn get_from_mock(key: &str) -> Result<Option<String>> {
            Ok(with_mock_storage(|storage| storage.get(key).cloned()))
        }
    ...
    fn test_accounts_metadata_management() {
        ...
        // メタデータの取得
        let retrieved_json = TestSecureStorage::get_from_mock(ACCOUNTS_KEY)
            .unwrap()
            .unwrap();
    ...
    fn test_multiple_accounts() {
        ...
        assert_eq!(
            retrieved_metadata.current_npub,
            Some("npub1bob".to_string())
        );
