...
#[tauri::command]
pub async fn initialize_p2p(state: State<'_, AppState>) -> Result<(), String> {
...
pub async fn join_p2p_topic(
    ...
    match &p2p_state.manager {
        Some(manager) => manager
            .join_topic(&topicId, initialPeers)
            .await
...
#[tauri::command]
pub async fn get_p2p_status(state: State<'_, AppState>) -> Result<P2PStatus, String> {
    let p2p_state = state.p2p_state.read().await;
...
#[tauri::command]
pub async fn get_node_address(state: State<'_, AppState>) -> Result<Vec<String>, String> {
    let p2p_state = state.p2p_state.read().await;

    match &p2p_state.manager {
        Some(manager) => manager.node_addr().await.map_err(|e| e.to_string()),
        None => Err("P2P manager not initialized".to_string()),
    }
}
