...
async fn test_peer_to_peer_messaging() {
    ...
    let node1 = GossipManager::new(iroh_secret_key1, secp_secret_key1, event_tx1)
        .await
        .unwrap();
    ...
    let node2 = GossipManager::new(iroh_secret_key2, secp_secret_key2, event_tx2)
        .await
        .unwrap();
    ...
    let timeout = tokio::time::timeout(Duration::from_secs(10), async {
        while let Some(event) = event_rx2.recv().await {
            if let P2PEvent::MessageReceived {
                topic_id: _,
                message,
                _from_peer: _,
            } = event
...
async fn test_multi_node_broadcast() {
    ...
    for _ in 0..3 {
        ...
        let node = GossipManager::new(iroh_secret_key, secp_secret_key, event_tx)
            .await
            .unwrap();
        nodes.push(node);
    ...
    let message = GossipMessage::new(MessageType::NostrEvent, test_payload.to_vec(), vec![]);
...
async fn test_duplicate_message_filtering() {
    ...
    // 同じメッセージを複数回送信
    let message = GossipMessage::new(MessageType::NostrEvent, b"Duplicate test".to_vec(), vec![]);
...
async fn test_peer_connection_stability() {
    ...
    node1.join_topic(&topic_id, vec![]).await.unwrap();
    node2
        .join_topic(&topic_id, node1_addr.clone())
        .await
        .unwrap();
...
async fn test_message_ordering() {
    ...
    for i in 0..message_count {
        let message = GossipMessage::new(MessageType::NostrEvent, i.to_le_bytes().to_vec(), vec![]);
        node.broadcast(&topic_id, message).await.unwrap();
