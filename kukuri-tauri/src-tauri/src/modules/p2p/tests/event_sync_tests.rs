...
mod tests {
    ...
    use crate::modules::p2p::message::{
        generate_topic_id, user_topic_id, GossipMessage, MessageType, GLOBAL_TOPIC,
    };
    ...
    async fn test_handle_gossip_message_other_types() {
        ...
        let mut message = GossipMessage::new(MessageType::Heartbeat, vec![1, 2, 3], vec![0; 32]);
    ...
    async fn test_duplicate_event_handling() {
        ...
        // 最初の処理でSentToP2Pに設定
        event_sync
            .test_set_sync_status(event.id.to_string(), SyncStatus::SentToP2P)
            .await;
    ...
    async fn test_kind_30078_topic_extraction() {
        ...
        let event = EventBuilder::new(nostr_sdk::Kind::from(30078u16), "Topic post content")
            .tags(vec![Tag::identifier("technology"), Tag::hashtag("tech")])
            .sign_with_keys(&keys)
            .unwrap();
