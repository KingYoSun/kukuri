...
impl HybridDistributor {
    ...
    pub async fn deliver_event(
        ...
        // 優先度に基づく戦略を選択
        let strategy = config.priority_strategies
            .get(&priority)
            .copied()
        ...
        let result = match strategy {
            DeliveryStrategy::P2POnly => {
                self.deliver_p2p_only(event).await
            }
            DeliveryStrategy::RelayOnly => {
                self.deliver_relay_only(event).await
            }
            DeliveryStrategy::Parallel => {
                self.deliver_parallel(event).await
            }
    ...
    pub async fn deliver_batch(
        ...
        let futures = events.into_iter().map(|(event, priority)| {
            self.deliver_event(event, priority)
        });
