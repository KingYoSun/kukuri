...
impl CommunityNodeHandler {
    ...
    async fn resolve_nostr_relay_urls_for_config(
        ...
        let cached_descriptors =
            sanitize_bootstrap_items(NODE_DESCRIPTOR_KIND, &cache.nodes.items, now, None);
        let has_bootstrap_nodes = config.nodes.iter().any(|node| node.roles.bootstrap);
        let mut live_descriptors: Option<Vec<serde_json::Value>> = None;
        let mut resolved = Vec::new();
        ...
        for node in &config.nodes {
            ...
            if node_relays.is_empty() && has_bootstrap_nodes && !has_matching_descriptor {
                if live_descriptors.is_none() {
                    live_descriptors = Some(
                        self.refresh_bootstrap_descriptors_for_config(config)
                            .await?,
                    );
                }
            ...
            if node_relays.is_empty()
                && should_fallback_to_base_url_nostr_relay(node, config, has_matching_descriptor)
                && let Some(relay_url) = build_nostr_relay_url_from_base_url(&node.base_url)
            {
                node_relays.push(relay_url);
            }

            for relay_url in node_relays {
    ...
    async fn refresh_bootstrap_descriptors_for_config(
        &self,
        config: &CommunityNodeConfig,
        ...
        let now = Utc::now().timestamp();
        let result = self
            .aggregate_bootstrap(nodes, "/v1/bootstrap/nodes")
            .await?;
        let items = sanitize_bootstrap_items(NODE_DESCRIPTOR_KIND, &result.items, now, None);

        let mut cache = self.load_bootstrap_cache().await?;
        cache.nodes = BootstrapCacheEntry {
            items: items.clone(),
...
fn collect_nostr_relay_urls_for_node(
    node: &CommunityNodeConfigNode,
    descriptors: &[serde_json::Value],
    now: i64,
) -> Vec<String> {
    let mut node_relays = Vec::new();

    for item in descriptors {
        if !descriptor_matches_base_url(item, &node.base_url, now) {
            continue;
        }

        for relay_url in extract_nostr_relay_urls_from_descriptor(item, now) {
            if !node_relays.contains(&relay_url) {
                node_relays.push(relay_url);
            }
        }
    }
...
fn collect_bootstrap_descriptor_nostr_relay_urls(
    descriptors: &[serde_json::Value],
    now: i64,
) -> Vec<String> {
    ...
    for item in descriptors {
        if !descriptor_has_role(item, "bootstrap", now) {
            continue;
        }
...
fn has_matching_bootstrap_descriptor(
    node: &CommunityNodeConfigNode,
    descriptors: &[serde_json::Value],
    now: i64,
) -> bool {
    descriptors
        .iter()
        .any(|item| descriptor_matches_base_url(item, &node.base_url, now))
}

fn descriptor_has_role(event_json: &serde_json::Value, expected_role: &str, now: i64) -> bool {
    let Some(event) = validate_kip_event_json(event_json, NODE_DESCRIPTOR_KIND, None, now) else {
        return false;
    };
    let Ok(content) = serde_json::from_str::<serde_json::Value>(event.content.trim()) else {
        return false;
    };
    ...
    content
        ...
        .map(|roles| {
            roles
                ...
                .any(|role| role.eq_ignore_ascii_case(expected_role))
        })
        .unwrap_or(false)
}

fn descriptor_matches_base_url(
    event_json: &serde_json::Value,
    expected_base_url: &str,
    now: i64,
) -> bool {
    extract_descriptor_http_base_url(event_json, now)
        .map(|base_url| base_url == expected_base_url)
        .unwrap_or(false)
}

fn extract_descriptor_http_base_url(event_json: &serde_json::Value, now: i64) -> Option<String> {
    let event = validate_kip_event_json(event_json, NODE_DESCRIPTOR_KIND, None, now)?;
    let content = serde_json::from_str::<serde_json::Value>(event.content.trim()).ok()?;
    let raw = content
        .get("endpoints")
        .and_then(|endpoints| endpoints.get("http"))
        .and_then(|value| value.as_str())?;
    normalize_base_url(raw).ok()
}
...
mod community_node_handler_tests {
    ...
    fn spawn_json_sequence_server_with_builder<F>(
        response_builder: F,
    ) -> (
        String,
        mpsc::Receiver<CapturedRequest>,
        ...
        F: FnOnce(&str) -> Vec<MockHttpResponse>,
    {
        let server = Server::http("127.0.0.1:0").expect("server");
        let base_url = format!("http://{}", server.server_addr());
        let responses = response_builder(&base_url);
        let (tx, rx) = mpsc::channel();
        let handle = thread::spawn(move || {
            for response_spec in responses.into_iter() {
                let request = match server.recv_timeout(MOCK_SERVER_RECV_TIMEOUT) {
    ...
    async fn resolve_nostr_relay_urls_prefers_matching_descriptor_ws_over_base_url_fallback() {
        ...
        assert_eq!(relay_urls, vec!["wss://relay.example/ws".to_string()]);
    ...
    async fn resolve_nostr_relay_urls_refreshes_live_bootstrap_descriptors_before_fallback() {
        ...
        let (base_url, rx, handle) = spawn_json_sequence_server_with_builder(|base_url| {
            ...
            let live_descriptor = build_bootstrap_descriptor_event_with_ws(
                base_url,
                Some("wss://relay.example/live"),
                &[],
            );
            let bootstrap_with_descriptor = json!({
                "items": [live_descriptor],
                "next_refresh_at": Utc::now().timestamp() + 600
            });
            vec![
                MockHttpResponse::json(200, bootstrap_without_descriptor),
                MockHttpResponse::json(200, bootstrap_with_descriptor),
            ]
        });
        ...
        let relay_urls = handler
            ...
            .await
            .expect("resolve nostr relays");
        assert_eq!(relay_urls, vec!["wss://relay.example/live".to_string()]);
    ...
    async fn resolve_nostr_relay_urls_errors_when_live_bootstrap_refresh_fails() {
        ...
        let (base_url, rx, handle) = spawn_json_server(bootstrap_without_descriptor);
        ...
        let err = handler
            .resolve_nostr_relay_urls_from_saved_config()
            .await
            .expect_err("live refresh should fail");
        assert!(matches!(
            err,
            AppError::Network(_) | AppError::NotFound(_) | AppError::Internal(_)
        ));
    ...
    async fn resolve_nostr_relay_urls_returns_empty_when_bootstrap_descriptor_has_no_ws() {
        ...
        let descriptor = build_bootstrap_descriptor_event_with_ws(&base_url, None, &[]);
        let descriptor_event: NostrEvent =
            serde_json::from_value(descriptor.clone()).expect("descriptor event");
        handler
            .ingest_bootstrap_event(&to_domain_event(&descriptor_event))
    ...
    #[tokio::test]
    async fn resolve_nostr_relay_urls_skips_base_url_fallback_for_non_bootstrap_nodes_in_multi_node_config()
     {
        let _env_guard = lock_bootstrap_env();
        ...
        let (bootstrap_base_url, rx, handle) =
            spawn_json_sequence_server_with_builder(|base_url| {
                let descriptor = build_bootstrap_descriptor_event_with_ws(
                    base_url,
                    Some("wss://relay.example/ws"),
                    &[],
                ...
                vec![
                    MockHttpResponse::json(200, response.clone()),
                    MockHttpResponse::json(200, response),
                ]
            });
        let api_base_url = "https://api.example.com".to_string();
        ...
        handler
            .set_config(CommunityNodeConfigRequest {
                nodes: vec![
                    CommunityNodeConfigNodeRequest {
                        base_url: bootstrap_base_url.clone(),
                        roles: Some(CommunityNodeRoleConfig {
                            labels: false,
                            trust: false,
                            search: false,
        ...
        assert!(
            !relay_urls
                .iter()
                .any(|value| value.contains("api.example.com/relay"))
        );
    ...
    async fn resolve_nostr_relay_urls_uses_bootstrap_descriptor_ws_when_http_endpoint_differs() {
        ...
        let descriptor_ws_url = "wss://relay.example.com/relay".to_string();
        let (base_url, rx, handle) = spawn_json_sequence_server_with_builder(|_base_url| {
            let descriptor = build_bootstrap_descriptor_event_with_ws(
                &descriptor_http_url,
        ...
        assert!(
            !relay_urls
                .iter()
                .any(|value| value.contains(&build_url(&base_url, "/relay")))
        );
