use crate::{
    presentation::{
        ...
        handlers::topic_handler::TopicHandler,
    },
    state::AppState,
};
use tauri::State;
...
pub async fn create_topic_v2(
    ...
    match state.topic_handler.create_topic(request).await {
...
/// すべてのトピックを取得する
#[tauri::command]
pub async fn get_topics_v2(
    state: State<'_, AppState>,
) -> Result<ApiResponse<Vec<TopicResponse>>, String> {
    match state.topic_handler.get_all_topics().await {
...
pub async fn join_topic_v2(
    ...
    match state.topic_handler.join_topic(request, &user_pubkey).await {
...
pub async fn leave_topic_v2(
    ...
    match state.topic_handler.leave_topic(request, &user_pubkey).await {
        Ok(_) => Ok(ApiResponse::success(())),
        Err(e) => Ok(ApiResponse::error(e.to_string())),
    }
}
...
/// トピックの統計情報を取得する
#[tauri::command]
pub async fn get_topic_stats_v2(
    state: State<'_, AppState>,
    request: GetTopicStatsRequest,
) -> Result<ApiResponse<TopicStatsResponse>, String> {
    match state.topic_handler.get_topic_stats(request).await {
