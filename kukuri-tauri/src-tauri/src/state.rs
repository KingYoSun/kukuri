...
use crate::modules::p2p::{GossipManager, P2PEvent, EventSync};
...
impl AppState {
    ...
    pub async fn initialize_p2p(&self) -> anyhow::Result<()> {
        ...
        self.event_manager.set_event_sync(Arc::clone(&event_sync_arc)).await;
            ...
            loop {
                ...
                if let Some(mut rx) = event_rx {
                    while let Some(event) = rx.recv().await {
                        match event {
                            P2PEvent::MessageReceived { topic_id, message, .. } => {
