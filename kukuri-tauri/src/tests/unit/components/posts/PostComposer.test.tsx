...
vi.mock('@/components/posts/MarkdownEditor', () => ({
  __esModule: true,
  default: ({
    value,
    onChange,
    ...
    <div>
      ...
      {onImageUpload && (
        <button
          ...
          onClick={() => onImageUpload(new File(['test'], 'test.jpg'))}
        >
          Upload Image
        </button>
      )}
...
vi.mock('@/components/posts/DraftManager', () => ({
  __esModule: true,
  default: ({
    onSelectDraft,
  }: {
    onSelectDraft: (draft: import('@/types/draft').PostDraft) => void;
  }) => (
