...
describe('LoginForm', () => {
  ...
  it('無効な形式の秘密鍵でログインしようとするとエラーを表示する', async () => {
    ...
    expect(toast.error).toHaveBeenCalledWith('無効な形式です。nsec1で始まる秘密鍵を入力してください');
    expect(mockLoginWithNsec).not.toHaveBeenCalled();
  });
  ...
  it('有効な秘密鍵でログインに成功する', async () => {
    ...
    // ログイン処理が呼ばれる
    await waitFor(() => {
      expect(mockLoginWithNsec).toHaveBeenCalledWith('nsec1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq');
    });
