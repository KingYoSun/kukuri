...
3D表示にfocusがある時、Tabで既存HUD、Enterでチャットを開き、固定を解除して対象UIへfocusを移す。UI内の通常Tab移動、Enter、IME入力を奪わない。EscapeでUIを閉じる場合はdraftを保持して3D表示へfocusを戻すが、自動で再固定しない。閉じるボタンで最後のUIを閉じた時は再取得を試み、拒否時は明示再開と視点調整ボタンを表示する。サークルメニューとカテゴリ構造は#1024で扱う。
