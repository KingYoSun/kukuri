...
- Scope revision: 2026-09-13-v2。追加要件を受け、v1の「ログアウト後は未ログイン画面」を撤回し、本書全体を更新した。
...
- 対象外: 完全削除、全端末ログアウト、サーバー側鍵失効、暗号形式変更、backup形式の再設計、workspace全体の再設計。「アカウント追加」は引き続き鍵importであり、鍵生成は初回起動と他アカウントがないlogoutの経路で行う。
