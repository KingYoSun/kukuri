...
- リスク区分: `C`
- 記録時点: 初回PR head監査のblocker修正後、修正後PR head確定前のworking tree。実装とlocal validationの証跡を固定し、再度のPR head独立監査とCIへ引き渡す段階。
- Goal: 端末バックアップ復元を、process停止、再起動、cancel、再同意を含む固定状態遷移で、再実行可能な旧状態、再同意待ちの新状態、または再同意後に確定した新状態へ収束させる。明示的な再同意前は復元runtimeとnetwork処理を開始しない。
...
初回PR head `e70d64a49ab99c22869c685ecb8864405ea418c8` の独立監査は`FAIL`となった。固定inventory 26件のうち、適合22件、不適合4件、未分類0件で、1件のblockerが既存の`AC-4`／`INVAR-2`、`INV-5`、`TR-7`、optional secret／Community Node network sinkにまたがっていた。

到達経路は、同じidentityでNode Bへ同意・認証した後にNode Bを現行設定から外し、Node Bを含む古いbackupでreplacement restoreするものだった。旧keyringのNode B consent／tokenが残り、復元後の`Auto` modeから再利用できた。またWindowsでは、旧directory移動後の存在しないDB pathからcredential名を再計算すると、移動前のcanonical pathと一致しない可能性があった。

固定完了条件を広げず、同じPRで次を修正した。

- productionのinstall本体へFakeKeyringを注入し、replacementで旧値をrollback用FileOnlyへ保存できない経路と、過去に使ったcanonical pathへの新規restoreでstale tokenが残る経路をfailing-beforeとして固定した。
- 固定private channel／gossip locatorと、staging／現行Community Node設定のtoken・invite・consent locatorの和集合を作る。
- replacementでは、その和集合にある旧値を旧directoryのFileOnlyへ退避してからtransactionを開始し、rollback後も旧状態を読めるようにする。
- 新規／replacementの双方で、stagingをfinal DB pathへ配置した後かつruntime構築前に、和集合のkeyring entryだけを削除する。これによりWindowsのcanonical pathを一致させ、backupに含まれるFileOnlyのprivate channel、gossip、inviteは保持する。
- default keyring自体が無い`NoDefaultStore`はentry不存在相当として扱い、それ以外の削除errorはfail-loudでrollbackする。

修正前に失敗した2件は修正後に成功した。修正後PR headはこの記録より後に固定し、同じ10 surface、9 transition、7 sensitive sinkだけで再度独立監査する。
...
| `INV-5` | `install_prepared_device_restore` | staging／現行設定のsecret locatorを先に固定し、replacementの旧値をFileOnlyへ退避する。Installing journal、旧directory退避、staging→final rename、canonical final pathのkeyring-only scrub、Installed journalの順で進み、途中errorは`error_with_restore_rollback`へ集約する。 | `replacement_crash_after_journal_before_old_move_preserves_original`、`replacement_stop_after_existing_directory_move_recovers_identity`、`new_account_install_stop_rolls_back_without_changing_existing_state`、`post_journal_move_error_and_installed_journal_error_rollback_immediately`、`replacement_restore_scrubs_stale_keyring_union_and_preserves_rollback_values`、`new_account_restore_scrubs_stale_keyring_at_reused_canonical_path` |
...
| --- | --- |
| staging／final／rollback directory | transaction開始入口は`restore_device_backup_command`だけ。変更処理は`prepare_device_restore`、`install_prepared_device_restore`、startup recovery、command失敗rollback、activation失敗rollbackへ分類した。 |
| identity／optional secret | restore書込みは`prepare_device_restore::persist_restored_secrets`と`install_prepared_device_restore::{make_account_file_self_contained,scrub_keyring_optional_secrets}`だけ。validationはFileOnlyでkeyring shadowを参照しない。installはstaging／現行Node locatorの和集合をcanonical final pathでkeyring-only scrubし、復元FileOnly値とrollback旧値を保持する。 |
...
- `AC-4`／`INVAR-2`: 初回PR head監査のblocker修正後working treeで適合。validation-only path、startup先行recovery、optional secretの旧keyring shadow除去、Committed consent reset、AwaitingConsent無runtime、全app IPC gate、background gate、再同意後activation、Activated後frontend markerの順序を固定した。
...
- core device backup targeted test: blocker修正前19件、修正後21件成功。
- identity targeted test: 11件成功。`NoDefaultStore`のkeyring-only削除を含む。
- frontend targeted test（App、DeviceBackupPanel、device backup API）: 15件成功。
- `apps/desktop/src-tauri`での`cargo test --lib`: 36件成功。
- `cargo xtask doctor`: 成功。
- `cargo xtask check`: 成功。format、workspace clippy、Tauri compile、frontend lint／typecheckを含む。
- `cargo xtask test`: 修正後working treeでRust 792件、harness 22件、frontend 141 files／1093件、doc test成功。Rust 3件は既定どおりskip。
- `cargo xtask scenario desktop_device_backup_restore`: 4 step成功。
- `cargo xtask rust-test`: 修正後working treeでRust 792件、harness 22件、doc test成功。Rust 3件は既定どおりskip。
...
初回PR headのCIでは、GitGuardian、LinuxのRust／smoke／Community Node／browser系8 laneが成功した。`linux-desktop-ui`は今回未変更の`DesktopShellPage.socialGraph.test.tsx`が5000msでtimeoutし、1093件中1092件が成功した。targeted再実行は4件すべて成功した。独立監査のblocker修正でheadを更新するため、この旧headのworkflowは`windows-fast`完了前に停止し、最終CIの根拠には使わない。
