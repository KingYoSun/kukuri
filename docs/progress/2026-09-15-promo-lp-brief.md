...
- Scope revision: 2026-09-18-promo-lp-brief-v6（v5からの変更: 配布候補を v0.2.7-preview.1 へ更新した。2026-09-18 の公開に合わせた操作者の依頼）。直前: 2026-09-18-promo-lp-brief-v5（v4からの変更: X の画像を #1041 の固定出力に合わせて 1600×900 の 1 枚から 1920×1080 の 2 枚へ変えた。各出力の設定の正本は `tools/promo/presets/stills.json`）。直前: 2026-09-18-promo-lp-brief-v4（v3からの変更: 配布候補を v0.2.6-preview.1 へ更新した。2026-09-18 の公開に合わせた操作者の依頼）。直前: 2026-09-18-promo-lp-brief-v3（v2からの変更: Dome予告を操作者が提供した実機の静止画1枚に限定し、動画から外した。S9の画面に限り、操作者本人とテスト用アカウントの名前の写り込みを許可した。いずれも 2026-09-18 の操作者の判断）
- 作業日: 2026-09-18
- 作業時の main: `d455bdd8e03541de9734f06a8e050a72bdb01bfd`
- 配布候補release: `v0.2.7-preview.1`（2026-09-18公開）。Issue起票時点の `v0.2.3-preview.2` から `v0.2.5-preview.3`、`v0.2.6-preview.1` を経て更新した（2026-09-18、公開に合わせて操作者が依頼）。LP が案内する release の正本は `apps/lp/release.json`。
- 配布候補commit: `e820382da627688c5b31f56fec168d0de09f0d9f`（tag `v0.2.7-preview.1` のpeel先。main の祖先）。#1039 の画面は `v0.2.6-preview.1`（`4b7519460e8baf3c29d7ec0eadc6eb2c727a1065`）の上から撮った。v0.2.7-preview.1 の変更は告知素材・LP・CI と、画面の見た目を変えない修正（Windows 初回起動、添付の取得ゲート、Metaverse）なので撮り直さない。
...
出典: `gh release view v0.2.7-preview.1`（2026-09-18取得）、[release一覧](https://github.com/kukuri-app/kukuri/releases/latest)、`CHANGELOG.md`、`README.md`。

| 事実 | 値 | 出典 |
| --- | --- | --- |
| tag | `v0.2.7-preview.1` | GitHub Release |
| 公開日時 | 2026-09-18T10:22:35Z | GitHub Release |
| commit | `e820382da627688c5b31f56fec168d0de09f0d9f` | `refs/tags/v0.2.7-preview.1^{}` |
| 表示バージョン | `0.2.7`（asset名）／`v0.2.7-preview.1`（tag） | asset名とtag |
...
| 対象 | asset | 備考 |
| --- | --- | --- |
| Windows 10 / 11 | `kukuri_0.2.7_x64-setup.exe`（+ `.sig`） | NSIS installer。x64のみ |
| Linux GUI | `kukuri_0.2.7_amd64.AppImage`（+ `.sig`）、`kukuri_0.2.7_amd64.deb`（+ `.sig`） | x86_64 / amd64のみ |
| Linux CLI | `kukuri-cli_0.2.7_x86_64-unknown-linux-gnu.tar.gz`、`kukuri-cli_0.2.7_aarch64-unknown-linux-gnu.tar.gz` | GUIとは別profile |
| 更新 | `latest-preview.json`、`windows-x86_64-updater-key.pub`、`linux-x86_64-updater-key.pub` | 更新manifestは署名あり |
| 検証・表示 | `SHA256SUMS.txt`、`THIRD_PARTY_NOTICES.md`、`release-provenance.json`、`kukuri_0.2.7_linux-native-compliance.json` | |
...
| 注記 | テスター向けのBuilder Preview（v0.2.7-preview.1）です。一般向けの安定版ではありません。 | This is a Builder Preview for testers (v0.2.7-preview.1), not a stable general release. |
...
| 版 | v0.2.7-preview.1 | v0.2.7-preview.1 |
...
- release表示: 画面内にバージョンが写る場合は `v0.2.7-preview.1` と一致させる。
...
| Windows | 上記releaseの `kukuri_0.2.7_x64-setup.exe` | 稼働中 |
| Linux AppImage / deb | 上記releaseの `kukuri_0.2.7_amd64.AppImage` / `kukuri_0.2.7_amd64.deb` | 稼働中 |
| Linux CLI | 上記releaseの `kukuri-cli_0.2.7_*.tar.gz` | 稼働中 |
...
1. 撮影対象releaseは撮影直前に再照合する。`v0.2.7-preview.1` より新しいpreviewが出た場合、#1040 などの撮影の着手時に再固定し、本書のScope revisionを更新する。LP のダウンロードは `apps/lp/release.json` と `apps/lp/scripts/sync-release.mjs` で更新する（`docs/runbooks/lp-publish.md`）。Dome の静止画（S9）は `v0.2.5-preview.3` の実機で撮ったもので、開発中の実験機能の予告として使い続ける。
