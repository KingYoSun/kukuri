...
計画中の CSAM / critical safety moderation 基盤（`docs/safety/community-node-critical-safety.md`）は、signed moderation event と trustness / relation risk signal を生成する。本文書は、それらが **network-wide command ではなく、issuer node の authority scope 内の判断、または各 node / client / user が任意に購読する optional trust input** であることを明確化する。

共通前提は `docs/architecture/p2p-first-community-node-responsibility-boundary.md`（P2P-first 責任境界）であり、manifest の authority scope、通報ルーティングとの連携もそこで固定した責任境界に従う。

この文書は法的助言ではなく、central governance model / network-wide moderation policy を導入するものでもない。そもそも kukuri は P2P を基盤としており、network 全体を統治する中央権者は構造的に存在し得ない。

## 1. 基本原則: advisory であって command ではない

kukuri には network 全体の moderation authority は存在しない。これは方針として置かないという選択ではなく、P2P 基盤上には network 全体を統治するチョークポイントが存在せず構造的に成立し得ないためである。したがって、ある node が発行した moderation event / safety advisory は、次のように扱う。
...
- kukuri project / default node は **global moderation authority ではない**。そのような authority は P2P 基盤上に存在し得ないため、default node が発行した moderation event も、他のあらゆる node の event と同じく optional trust input にすぎない。
...
CSAM / CSE / malware / phishing 等の critical safety であっても、「network 全体に強制する中央権者」が現れるわけではない。そのような中央権者は P2P 基盤上に存在し得ないからである。各 node は自分の index / discovery / recommendation から critical content を**自分の判断で**排除し、その判断を signed moderation event として説明・監査可能にする。受け手側が critical safety signal を強く重み付けるのは自然だが、それは受け手の採用判断であり、network-wide command ではない。
...
issuer node が manifest で宣言した `authority_scope` の範囲。moderation event の効果はこの範囲に限定される。具体的には:
...
critical safety architecture（`docs/safety/community-node-critical-safety.md`）の model と整合する 3 段階。signal がどこまで配布されるかを示す。
...
## 5. report routing との連携

risk signal が通報対象に関与している場合、分散通報ルーティングは issuer node を **report target の候補**として含める。
...
- これは content provenance の `observedVia`（capability = `moderation` / `trust_signal`）として表現され、report routing が manifest authority scope と突き合わせて候補化する。

report routing は、中央集約できる network-wide 窓口が P2P 基盤上に存在しないことを前提に、issuer node の authority scope 内へ通報を route する。
...
- 「default node がある」ことは、「default node が network 全体の moderation 権者である」ことを意味しない。P2P 基盤上ではそのような権者自体が成立し得ない。
...
- provenance 不明時に moderation 判断を default node へ fallback して帰属させない（content provenance / 分散通報ルーティングの不変条件と整合）。
...
- local / subscribed nodes / public の使い分けが critical safety architecture と整合している → 2（visibility 表）
...
- community node manifest の authority scope / P2P boundary
- public manifest endpoint
- content provenance / responsible capability metadata
- 分散通報ルーティング
- `docs/safety/community-node-critical-safety.md`（Public community node 向け CSAM / critical safety moderation 基盤, この signal の生成側）
