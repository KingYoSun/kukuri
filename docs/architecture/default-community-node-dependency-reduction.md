...
kukuri には onboarding のための **default community node** が存在する。これは初期利用体験のために必要な妥協だが、**default node があることは kukuri project が network-wide authority であることを意味しない**。

この文書は、default community node への暗黙依存を段階的に減らす roadmap を固定する。前提は `docs/architecture/p2p-first-community-node-responsibility-boundary.md`（P2P-first 責任境界, #354）であり、user identity / profile / social graph は default node に所有されない。

この文書は法的助言ではなく、central node registry を導入するものでもない。
...
- default node は **network-wide authority ではない**。「default である」ことは「kukuri network 全体の運営者・通報先・moderation 権者・凍結権者である」ことを意味しない。
...
- user identity / profile / social graph は default node に所有されない。default node が停止・変更・削除されても、これらは失われない（#354, #361）。
...
| future index / moderation / trust signal | 低（未実装 / 計画中, Phase B） | 補助かつ optional trust input（#362）。 |
| generated policy / manifest display | 低（表示のみ） | 補助。manifest 取得失敗時は fallback 表示（#357）。 |
...
Phase 1: settings で node dependency visibility を表示
  ...
  - default node が network-wide authority ではないことを明示する。
...
- Phase 1 の基盤（settings の node 依存度 / capability scope / authority scope 表示）は #357 で実装済み。identity / profile / social graph が node-owned ではないこと、default node が network-wide authority ではないことを settings で明示している。
- Phase 2 の前提（manifest authority scope #355 / public manifest endpoint #356 / content provenance #358 / 分散通報ルーティング #310）が揃っており、user-added node を first-class に扱うための土台はある。
...
- default node は network-wide authority ではない。停止しても network は継続する。

## 受け入れ条件との対応

- default node dependency reduction roadmap が文書化されている → 3
- default node が onboarding infrastructure であり network-wide authority ではないことを明記している → 1
...
- #354 の P2P-first responsibility boundary と整合している → 全体
...
- client settings の node 依存度表示（#357, Phase 1 実装）
- content provenance / responsible capability metadata（#358）
- community node shutdown と user continuity（#361）
