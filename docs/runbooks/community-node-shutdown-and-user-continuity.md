...
kukuri の community node は Mastodon 的な home server ではなく、P2P-first network の補助層である（前提: `docs/architecture/p2p-first-community-node-responsibility-boundary.md`, #354）。したがって shutdown も「アカウントの引っ越し」ではなく「補助 capability の停止」として設計する。

関連:
- community node manifest / authority scope / capability scope（#355）
- public manifest endpoint（#356）
- client settings の node 依存度表示（#357）
- operator docs generator（#352）
...
| node-local moderation labels | この node の moderation label が参照できなくなる | 別 node の label（optional trust input, #362） |
...
重要: これらはいずれも **node-local な補助出力**であり、停止しても user の identity / profile / social graph という canonical state には影響しない。moderation labels / trust signals は元々 optional trust input（#362）であり、network-wide な認定ではない。
...
2. **public listing / default listing からの撤退**。default-onboarding-node に登録されている場合は、その登録元から外す手続きを行う（network-wide authority ではないため、外しても network は継続する）。
3. **capability を段階的に停止**する。利用者影響の小さいものから順に止めるとよい。各 capability の停止方針:
   ...
   - report endpoint（#370）: 通報受付を止める。abuse contact は最終窓口として案内を残すか、shutdown notice に移行先を記載する。
4. **retention policy に沿ってデータを扱う**。connection logs / moderation logs / report data は、生成された operator docs（data-retention-policy.md, #352）の保持期間に従って削除または保全する。signed moderation event は監査用に保持方針を明示する。
5. **generated operator docs の final export** を用意する（#352 の `cargo run -p cn-operator -- generate-docs`）。停止時点の manifest / policy / retention を最終版として保存し、説明責任を果たせるようにする。
...
- report data（#370）: 保持期間に従う。未対応の通報がある場合は移行先 / abuse contact を shutdown notice に記載する。
- signed moderation event: issuer node の判断記録として、監査可能性のために保持方針を明示する（即時削除 / 一定期間保持のいずれか）。

## 3. Manifest shutdown notice の形

manifest（#355）に shutdown notice を載せ、client が事前・事中に影響を表示できるようにする。提案する形:
...
shutdown notice は manifest の他フィールド同様 unauthenticated に取得できる（#356）。private secret は含めない。

## 4. Client UX

client は manifest の shutdown notice と capability scope（#357 の node 依存度表示）を使って、次を行う。
...
- **identity / social graph は node-owned ではない**ことを説明する（停止しても user は失われない、という安心を明示する。#357 の境界説明と同じトーン）。

client は、shutdown を「アカウント凍結 / 退会」ではなく「補助 capability の停止と代替手段の案内」として表示する。

## 5. #352 generated docs との連携

- 停止時の説明責任は #352 の生成文書で果たす。final export（manifest / terms / privacy / retention / abuse-policy / moderation-policy）を停止時点版として保存する。
