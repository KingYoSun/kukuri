...
- Caddy 内部 auto-TLS は使わない。`cn-iroh-relay` の QUIC が PEM ファイルを直接読むため、
  ACME companion（certbot）が `api_domain` / `relay_domain` の PEM を `/opt/kukuri/certs` に
...
VM 内のサービスは `/opt/kukuri/community-node` の docker compose で動く。SSH は IAP のみ
