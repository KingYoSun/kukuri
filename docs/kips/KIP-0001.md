...
- Trust は **署名付き提案（assertion）** として配布し、スコア押し付けを避ける。
...
- `30382-30385` **NIP-85 assertions** (addressable)
- `10040` **NIP-85 trust provider list** (replaceable 推奨)
...
## 5. Trust = NIP-85 Assertions

### 5.1 NIP-85 assertion (kind=30382-30385)

署名主体（provider）が subject について claim を発行する。
...
tags（例）:
- `["d","<identifier>"]`
- `["claim","reputation|moderation.risk|capability"]`
- `["rank","0-100"]`
- `["expiration","<unix_ts>"]`
- `["t","<topic_id>"]`（任意）

### 5.2 NIP-85 trust provider list (kind=10040)

ユーザーが「どの provider を採用するか」を宣言する。

tags（例）:
- `["30382:rank","<provider_pubkey_hex>","<relay_url?>"]`
- `["30383:rank","<provider_pubkey_hex>","<relay_url?>"]`
...
join 申請イベント。招待capabilityを提示し、参加希望を通知する（friend/friend_plus は招待なしで送信可）。
...
- friend_plus は **pull join.request** を正とし、受理側が FoF 判定（相互フォロー 2-hop）で承認した場合のみ配布する。

## 7. Client Rules (Minimal)

- ノードは **提案者**。label/assertion の採用はユーザー設定に依存。
