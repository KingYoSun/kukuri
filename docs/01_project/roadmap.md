...
| P2P & Discovery | Mainline DHT + Gossip の運用 Runbook を整え、EventGateway 経由でアプリ層へ隠蔽 | EventGateway ポート/mapper, `P2PStack` DI, Runbook Chapter10, CLI `apply_cli_bootstrap_nodes` PoC | `phase5_dependency_inventory_template.md`, `docs/03_implementation/p2p_mainline_runbook.md` | ✅ Gateway/Stack 抽象化後、`p2p_mainline_smoke.rs`・`scripts/test-docker.ps1 integration` と Runbook を同期。`p2p_metrics_export` でノード状態を定期採取。 |
