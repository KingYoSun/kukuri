...
- **FR-031**: クライアントは BitTorrent Mainline DHT / `kukuri-cli` が提供するブートストラップからピアリストを取得できる
...
- **NFR-031**: ピア発見の冗長性（複数の Mainline DHT ブートストラップノード / `kukuri-cli` エクスポートを用いる）
...
- irohによるP2P通信
- BitTorrent Mainline DHT + `kukuri-cli` によるピア発見
...
- 2025年11月21日: Discovery を Mainline DHT + `kukuri-cli` ブートストラップに統一（v1.1.0）
