...
- 残るギャップ: `/trending` `/following` は `trending_metrics_job`（AppState 5分間隔）と Docker `trending-feed` シナリオまで整備済み（Inventory 5.7 / `tasks/completed/2025-11-08.md`）。DM 導線も `direct_message_conversations` テーブル・Kind4 IPC・`DirectMessageInbox` の仮想スクロール/候補補完/未読共有を実装し、`Header.test.tsx` / `DirectMessageInbox.test.tsx` / `useDirectMessageBadge.test.tsx` で監視している。設定モーダルは `update_privacy_settings` コマンド + `20251109093000` マイグレーションで Stage2（プライバシー永続化）を完了し、残課題は Stage3 の Doc/Blob 連携（Inventory 5.1 + Sec.6）。未完リストは、(1) Global Composer からのトピック作成ショートカットと `OfflineActionType::CREATE_TOPIC`（Inventory 5.9）、(2) `delete_post` 後の React Query キャッシュ整合性（Inventory 5.10）、(3) `join_topic_by_name` / `delete_events` / `add_relay` など未配線 Tauri API の棚卸し、(4) `sync_engine` の再送ログ + Offline UI/UX Stage4.4、(5) Runbook/CI への Service Worker 手順の反映が中心。
...
| 領域 | 目的 | 現状 | 次アクション | 参照 |
| --- | --- | --- | --- | --- |
| ユーザーフロー / UX | `/trending` `/following` `/profile/$userId` `/direct-messages` `/search` の導線を「稼働中」に統一 | Summary Panel・DM Inbox・ユーザー検索・設定モーダル（Stage2）は ✅。`phase5_user_flow_summary.md` で残る未完チェックは 5.1 Stage3（Doc/Blob）、5.9（トピック作成ショートカット）、5.10（投稿削除キャッシュ）に集約。 | `tauri_app_implementation_plan.md` Phase3 の「MVP残タスク」テーブルに従い、5.9/5.10/6（プロフィール Stage3）をクローズ → Post-MVP として 3.3 (リアクション) を維持。 | `phase5_user_flow_inventory.md`, `phase5_user_flow_summary.md` |
