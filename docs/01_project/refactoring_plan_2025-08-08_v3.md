...
- 残るギャップ: `/trending` `/following` は `trending_metrics_job`（AppState 5分間隔）と Docker `trending-feed` シナリオまで整備済み（Inventory 5.7 / `tasks/completed/2025-11-08.md`）。DM 導線も `direct_message_conversations` テーブル・Kind4 IPC・`DirectMessageInbox` の仮想スクロール/候補補完/未読共有を実装し、`Header.test.tsx` / `DirectMessageInbox.test.tsx` / `useDirectMessageBadge.test.tsx` で監視している。設定モーダルは `update_privacy_settings` コマンド + `20251109093000` マイグレーションで Stage2（プライバシー永続化）を完了し、Stage3（Doc/Blob + privacy）は 2025年11月10日に `profile_avatar_sync` + `useProfileAvatarSync` 常駐フックを実装済み。未完リストは、(1) Global Composer からのトピック作成ショートカットと `OfflineActionType::CREATE_TOPIC`（Inventory 5.9）、(2) `delete_post` 後の React Query キャッシュ整合性（Inventory 5.10）、(3) `join_topic_by_name` / `delete_events` / `add_relay` など未配線 Tauri API の棚卸し、(4) `sync_engine` の再送ログ + Offline UI/UX Stage4.4、(5) Runbook/CI への Service Worker 手順の反映が中心。
...
| 領域 | 目的 | 現状 | 次アクション | 参照 |
| --- | --- | --- | --- | --- |
| ユーザーフロー / UX | `/trending` `/following` `/profile/$userId` `/direct-messages` `/search` の導線を「稼働中」に統一 | Summary Panel・DM Inbox・ユーザー検索・設定モーダル（Stage2/Stage3 Doc/Blob + privacy）は ✅。`phase5_user_flow_summary.md` で残る未完チェックは 5.9（トピック作成ショートカットのテスト再実行）、5.10（投稿削除キャッシュ）、Summary Panel → `trending_metrics_job` の自動化。 | `tauri_app_implementation_plan.md` Phase3 の「MVP残タスク」テーブルに従い、5.9/5.10 は 2025年11月10日に実装反映済み（再テスト待ち）、プロフィール Stage3 はクローズ → Post-MVP として 3.3 (リアクション) を維持。 | `phase5_user_flow_inventory.md`, `phase5_user_flow_summary.md` |
| sync_queue / Offline | 楽観更新＋競合解決で多端末利用時のデータ整合性を確保 | テーブル設計と `useSyncManager` の TODO が残置。`OfflineService` まわりで Stage3 が未着手。 | Phase4.1〜4.4 を MVPタスクに昇格（`tauri_app_implementation_plan.md` 参照）。`sync_queue`/`offline_actions` テーブルの migration と `sync_offline_actions` API、UI の conflict banner/Retry を実装し、`pnpm vitest src/stores/offlineStore.test.ts` を拡張。2025年11月09日: `list_sync_queue_items` と再送キュー履歴 UI を追加し、`npx vitest run src/tests/unit/hooks/useSyncManager.test.tsx src/tests/unit/components/SyncStatusIndicator.test.tsx src/tests/unit/components/OfflineIndicator.test.tsx` で検証。残タスクは Phase4.4（OfflineIndicator の簡素化・Runbook 連携）と `sync_engine` 側での再送結果ログ。 | `tauri_app_implementation_plan.md` Phase4 |
