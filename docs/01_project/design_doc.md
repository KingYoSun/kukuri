...
## ドキュメント情報
- **プロジェクト名**: kukuri
- **バージョン**: 1.3.2 (Stage4: プロフィール Service Worker / Offline sync / trending metrics artefact 反映)
- **最終更新日**: 2025年11月13日
- **作成者**: Grok AI (xAI)
...
| --- | --- | --- | --- |
| 体験/UX | `/trending` `/following` `/profile/$userId` `/direct-messages` の主要フローをブロッカーなしで提供 | Summary Panel/DM未読/プロフィール編集/検索/Topic作成導線が全て接続され、`phase5_user_flow_summary.md` の 3状態（稼働/改善/未実装）で「未実装」が0件になる | Stage4（プロフィール Service Worker + Offline ログ / Topic・Post Offline 導線）を 2025年11月12〜13日に完了し、`tmp/logs/profile_avatar_sync_stage4_<timestamp>.log` / `nightly.profile-avatar-sync`、`tmp/logs/topic_create_host_20251112-231141.log` / `tmp/logs/topic_create_20251112-231334.log` / `test-results/topic-create/20251112-231334-*.json`、`tmp/logs/post_delete_cache_20251113-085756.log` / `tmp/logs/post-delete-cache_docker_20251113-002140.log` / `test-results/post-delete-cache/20251113-002140.json` を Nightly artefact として固定。残タスクは DM Inbox 既読共有の contract テストと `/search` レートリミット UI。 |
| P2P & Discovery | Mainline DHT と Gossip 経由で恒常的に接続し、イベント配送とRunbookを提供 | `docs/03_implementation/p2p_mainline_runbook.md` のRunbook完成、`bootstrap` CLI/Workersの動的更新、EventGateway抽象化の完了 | 2025年11月13日に EventGateway ポート実装と `P2PStack` trait 化を完了（`tmp/logs/cargo-test-kukuri-tauri_di_20251113.log`, `tmp/logs/test-docker-rust_di_20251113.log`, `tmp/logs/cargo-test-kukuri-cli_di_20251113.log`）。Runbook Chapter10 と RelayStatus → `apply_cli_bootstrap_nodes` PoC（`tmp/logs/relay_status_cli_bootstrap_20251112-094500.log`）を同期済み。残りは `docs/01_project/deprecated/refactoring_plan_2025-08-08_v3.md` Phase5 で定義した Runbook KPI（Offline sync_queue + DHT 再接続メトリクス）の定常監視を仕上げる。 |
| データ/同期 | Offlineファースト（sync_queue/楽観更新）とトレンド指標の自動計測を統合 | `tauri_app_implementation_plan.md` Phase4 の sync_queue / offline_actions / conflict UI 完了、`trending_metrics_job` + Docker `trending-feed` シナリオが green | Stage4（2025年11月11日）で `cache_metadata` Doc/Blob 拡張・Service Worker・`./scripts/test-docker.{sh,ps1} ts --scenario offline-sync --no-build` が `tmp/logs/sync_status_indicator_stage4_<timestamp>.log` / `test-results/offline-sync/*.json` として固定済み。`trending_metrics_job` 監視も `tmp/logs/trending_metrics_job_stage4_<timestamp>.log` / `test-results/trending-feed/prometheus/` を Nightly artefact 化。残タスクは `sync_engine` 再送メトリクスと `nightly.user-search-pagination` の安定化。 |
| オペレーション/テスト | CI/CD・NightlyでMVP体験を再現し、失敗時にRunbookで復旧可能 | `github/test.yml` の `native-test-linux` / `format-check` / `nightly trending-feed` が安定し、`scripts/test-docker.{sh,ps1}` でts/lint/rustシナリオをローカル再現可能 | `nightly.topic-create` / `nightly.post-delete-cache` / `nightly.profile-avatar-sync` / `nightly.user-search-pagination` の artefact を Runbook と同期済み。残タスクは GitHub Actions 上の `pnpm vitest` キャッシュ・権限（Docker/Windows）と DM contract テスト追加。`tasks/status/in_progress.md` (GitHub Actions) に従い、ログパスを `docs/01_project/roadmap.md` / `phase5_ci_path_audit.md` へ反映する。 |
