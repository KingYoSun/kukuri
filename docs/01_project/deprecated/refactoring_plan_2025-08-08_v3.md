# リファクタリング計画（改善版）
作成日: 2025年08月08日
最終更新: 2025年11月19日（アーカイブ完了）

> **ステータス:** 2025年11月19日時点ですべてのアクションアイテムを完了し、
> 現在は履歴参照目的のみで保管しています。最新のロードマップやタスクは
> `docs/01_project/roadmap.md` と `docs/01_project/activeContext/` を参照してください。
...
- **目的と前提**  
  - Phase 1〜4 で整理済みの TODO/DRY 対応が完了している状態を前提とし、`docs/01_project/deprecated/refactoring_plan_2025-08-08_v3.md` に定義したレイヤ構成への移行を完了させる。  
...
- [x] dead_codeのうち80%以上が削除または使用開始
  ...
  - 2025年11月19日: `rg '#\[allow(dead_code)' -g '*.rs'` で 7 件あった `#![allow(dead_code)]` を `application` / `shared` / `presentation` / `infrastructure` / `domain` / `application/shared/tests` から削除し、未参照だった `src-tauri/src/modules/**/*` はディレクトリごと撤去。`p2p_metrics_export` が `test_support` に依存せず `TopicMetricsRepository` / `SqliteRepository` / `ConnectionPool` / `AppConfig` を直接利用できるよう `lib.rs` で再エクスポートし、`./scripts/test-docker.ps1 rust`（`tmp/logs/test_docker_rust_20251119-055648.log`）で 190 テストの成功を確認。`phase5_ci_path_audit.md` / `p2p_mainline_runbook.md` に作業内容と `tmp/logs/dead_code_inventory_20251119-053142.log` を記録。
