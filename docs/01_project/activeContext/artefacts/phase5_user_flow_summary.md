...
- デバッグ専用 `clear_all_accounts_for_test` は UI から露出させず、DevTools/QA ユーティリティから `SecureStorageApi.clearAllAccountsForTest` を呼び出す時のみ使用する。実行ログを `tmp/logs/dev_account_reset_<timestamp>.log` に残し、利用理由・担当者を記録するルールを定義して誤操作を防ぐ。
