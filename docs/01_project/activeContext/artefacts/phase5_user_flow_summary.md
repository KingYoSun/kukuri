...
- **Mainline DHT / EventGateway**: Runbook Chapter10 / RelayStatus リンクに加え、2025年11月11日に `kukuri-cli --export-path` が出力する `cli_bootstrap_nodes.json` を `get_relay_status` / `apply_cli_bootstrap_nodes` 経由で適用する PoC を実装。`phase5_dependency_inventory_template.md` の P2P 行も更新済みで、Gateway/mapper のDI差分は `phase5_event_gateway_design.md` に反映中。
