...
| `/search`（Inv 5.8） | `useUserSearchQuery` による Infinite Query / ソート（関連度・最新順） / `allow_incomplete` フォールバック / レートリミット UI / SearchBar 警告スタイルを実装済み。DM Inbox など他画面も同一 API を再利用。 | 2 文字未満の補助検索・`retryAfter` 自動復帰の Nightly 化・`SearchErrorState` の artefact 保存が未整備。`phase5_ci_path_audit.md` に記載したログを progressReports へ紐付ける必要がある。 | `phase5_user_flow_inventory.md` 5.8、`docs/01_project/progressReports/nightly.partial-feature-usage.md#search` | `nightly.user-search-pagination`（`./scripts/test-docker.{ps1,sh} ts --scenario user-search-pagination --no-build`）、`user-search-pagination-logs` / `user-search-pagination-log-archive` / `user-search-pagination-reports` artefact、`tmp/logs/user_search_pagination_*.log` |
| SyncStatusIndicator / Offline Sync（Inv 5.11） | Stage4 で Doc/Blob 対応 `cache_metadata` マイグレーション・Service Worker・Doc/Blob 競合バナー・`offline-sync` Docker シナリオを実装。ヘッダー CTA と `OfflineIndicator` も Stage4 仕様で統一。 | Topic/Post/Follow など Doc/Blob 以外の OfflineAction を Nightly に含める計画、`errorHandler` の `SyncStatus.*` 拡張、`profile-avatar-sync` 以外の Service Worker シナリオが backlog。 | `phase5_user_flow_inventory.md` 5.11、`docs/01_project/progressReports/nightly.partial-feature-usage.md#syncstatusindicator` | `nightly.sync-status-indicator`（`./scripts/test-docker.{ps1,sh} ts --scenario offline-sync --no-build`）、`sync-status-indicator-logs` / `test-results/offline-sync/*.json`、`profile-avatar-sync-logs`（Doc/Blob テレメトリ連動） |
...
| B | 鍵管理ダイアログ | 2025年11月17日: 設定>アカウント>鍵管理ボタンから `KeyManagementDialog` を起動し、エクスポート/インポートと履歴表示を提供。Runbook Chapter4 にバックアップ手順を追記。 | 端末故障時に復旧不能。運用リスク高。 | `KeyManagementDialog` 実装（`TauriApi.exportPrivateKey` + `dialog.save`/`fs.writeTextFile` + クリップボードコピー）、`useKeyManagementStore` で操作履歴を `withPersist` に保存、`./scripts/test-docker.ps1 ts` / `./scripts/test-docker.ps1 rust -Test key_management` で UI/Rust の契約テストを追加。 |
