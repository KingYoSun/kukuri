...
- **ユーザー検索**: `useUserSearchQuery` と `UserSearchResults` に cursor/sort／レートリミット UI／無限スクロールを実装し、2025年11月10日に関連度/最新順トグルと `sort` オプションのテストを `npx pnpm vitest run src/tests/unit/hooks/useUserSearchQuery.test.tsx src/tests/unit/components/search/UserSearchResults.test.tsx` で追加。残課題は `allow_incomplete` フォールバックと SearchBar 側の警告スタイル、Docker `user-search-pagination` シナリオの起票。
...
| ユーザー | `/search` (users) | `search_users` で実ユーザー検索、フォロー/解除ボタン | 改善中 | フォロー結果と Infinite Query は稼働中。2025年11月10日に関連度/最新順トグルとレートリミット UI を実装し、`npx pnpm vitest run src/tests/unit/hooks/useUserSearchQuery.test.tsx src/tests/unit/components/search/UserSearchResults.test.tsx` で検証済み。`allow_incomplete` フォールバックと Docker `user-search-pagination` シナリオは backlog。 |
...
| B | ユーザー検索導線改善 | `/search` (users) は `search_users` で実ユーザーを表示し、Infinite Query・レートリミット UI・関連度/最新順トグルまで実装済み（2025年11月10日）。`allow_incomplete` フォールバック・SearchBar 警告スタイル・Docker `user-search-pagination` シナリオが未着手。 | ソートやページネーションは利用できるが、短い入力時の補助検索と回帰テストの自動化が不足。 | Inventory 5.8 に沿って `allow_incomplete` 連携と SearchBar のエラーハイライトを実装し、`npx pnpm vitest …` + Docker シナリオで証跡を残す。 |
