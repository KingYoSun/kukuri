...
| --- | --- | --- | --- | --- |
| UX/体験導線 | `/trending` `/following` `/profile/$userId` `/direct-messages` `/search` を横断し、サイドバー/グローバルコンポーザーから同一導線を完走できる状態 | `DirectMessageInbox` の既読共有・会話検索、`UserSearchResults` の Docker `user-search-pagination` シナリオ、オフライン Topic 作成（`OfflineActionType::CREATE_TOPIC`）と Post 削除 `post-delete-cache` Docker シナリオが未完。プロフィール Stage4（Service Worker + Offline ログ）は 2025年11月12日に完了し、`tmp/logs/profile_avatar_sync_stage4_<timestamp>.log` / `profile-avatar-sync-logs` artefact が Nightly へ移行済み。 | Stage4 完了後は DM/Search/Topic/Post 導線の backlog（`phase5_user_flow_inventory.md` Sec.5.4/5.6/5.7/5.9/5.10/5.11）を順次解消。まず `topic_create` / `post_delete_cache` のオフライン再送と `user-search-pagination` Docker シナリオを仕上げ、`phase5_ci_path_audit.md`・Runbook を更新する。 | `phase5_user_flow_inventory.md` Sec.5.1/5.4/5.7/5.9/5.10/5.11, `tauri_app_implementation_plan.md` Phase3 |
