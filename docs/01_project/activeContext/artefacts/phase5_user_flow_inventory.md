...
- 今後の改善タスク（導線不足／未接続 API）を `docs/01_project/deprecated/refactoring_plan_2025-08-08_v3.md`（アーカイブ済み）へ反映するためのインプットを提供する。
...
| セグメント | 使用できている範囲 | 未接続/残タスク | テスト / Artefact / Runbook |
| --- | --- | --- | --- |
| SyncStatusIndicator / OfflineIndicator | Stage4 時点で Doc/Blob 対応 `cache_metadata`、Service Worker（`profileAvatarSyncSW.ts`）、BroadcastChannel 同期、Doc/Blob 競合バナー、`offline-sync` Docker シナリオを整備済み。ヘッダー CTA と `OfflineIndicator` は同じ `useSyncManager` / `useOfflineStore` を共有し、`get_cache_status` ポーリング・`add_to_sync_queue` CTA も UI 上で完結する。Stage5.1 では `pendingActionSummary` UI を追加し、`errorHandler.info('SyncStatus.queue_snapshot' | '...pending_actions_snapshot' | '...retry_metrics_snapshot')` を通じて `sync_queue` / `offline_actions` / `OfflineRetryMetrics` を Nightly ログへ記録する。 | ✅ Topic/Post/Follow/DM の OfflineAction を Nightly シナリオに含め、`tmp/logs/sync_status_indicator_stage4_{topic,post,follow,dm}_<timestamp>.log` と `test-results/offline-sync/{topic,post,follow,dm}/${timestamp}-*.json` を artefact 化（`sync-status-indicator-{topic,post,follow,dm}`）。`offlineSyncTelemetry.test.tsx` でカテゴリごとの UI/テレメトリを固定し、`phase5_user_flow_summary.md` backlog を解消した。 | `./scripts/test-docker.{ps1,sh} ts --scenario offline-sync --offline-category <cat> --no-build`、`profile-avatar-sync-logs`（Doc/Blob テレメトリ）と `docs/01_project/progressReports/nightly.partial-feature-usage.md#syncstatusindicator` のトリアージ手順。 |
