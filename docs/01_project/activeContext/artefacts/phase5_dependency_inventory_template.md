...
| PostService | `application/services/post_service.rs` | `domain::entities::{Post,Event,User}`, `application::ports::cache::PostCache`, `infrastructure::database::PostRepository`, `infrastructure::p2p::EventDistributor`, `application::services::event_service::EventServiceTrait` | Application | Medium | 2025年10月27日: `PostCache` ポートを追加し、キャッシュ実装の DI を `AppState` に集約。publish 成功/失敗の整合性をユニットテストで検証。2025年10月31日: トピック別投稿キャッシュの TODO を解消し、`PostCache` 実装とテストを拡張するタスクが残存。 |
