...
16. **Ops/CI: Nightly & GitHub Actions で MVP 導線を安定再現**  
    ...
    - メモ (2025年11月14日): `cargo fmt` ずれで `Format Check` が失敗していたため `kukuri-tauri/src-tauri/src/presentation/commands/p2p_commands.rs` を整形し、`gh act --workflows .github/workflows/test.yml --job format-check --container-options "--user root"` でローカル再現・緑化を確認済み。`docker-test` ジョブの artefact 不足は別途要調査。
