...
14. **Ops/CI: Nightly & GitHub Actions で MVP 導線を安定再現**  
    ...
    - 子タスク:  
        ...
        - ~~14-4: `gh act` を用いた CI failure 再現手順の Runbook 追記~~（2025年11月17日完了。`docs/01_project/progressReports/gh-act_ci-runbook.md` に `--bind` + `--container-options "--user 0"` / `--privileged` / `--artifact-server-path tmp/act-artifacts` を揃えたコマンド雛形を整理し、`gh act --bind -W .github/workflows/test.yml -j format-check --container-options "--user 0" -P ubuntu-latest=ghcr.io/catthehacker/ubuntu:full-latest > tmp/gh-act-format-check.log` および `gh act --bind -W .github/workflows/nightly.yml -j profile-avatar-sync --container-options "--privileged --user 0" -P ... --artifact-server-path tmp/act-artifacts > tmp/gh-act-profile-avatar-sync.log` の両方でグリーンを確認。`Dockerfile.test` の `chmod +x scripts/docker/ts-test-entrypoint.sh` や `actions/upload-artifact` 対策も Runbook へ反映し、Nightly artefact との行き来を README / nightly.index から辿れるようにした。）  
    ...
    - 更新 (2025年11月17日): `gh run view 19416727459 --log` で `KeyManagementDialog` 周りの `recordAction` not a function と `@tauri-apps/plugin-dialog` の `open` 衝突が CI を止めていることを特定し、`kukuri-tauri/src/tests/setup.ts` と `KeyManagementDialog` 系の修正方針を整理。  
    - 更新 (2025年11月17日-2): `gh act -W .github/workflows/test.yml -j format-check --container-options "--user 0"` と `gh act -j native-test-linux --container-options "--user 0"` をローカルで完走させ、`cargo fmt` / `pnpm prettier --write src/components/settings/KeyManagementDialog.tsx` / Vitest / tsc / ESLint がグリーンになることを確認。  
    - ���� (2025�N11��16��): Nightly �Ő��s���Ă���� `scripts/test-docker.sh` �̍K�����ƃu���p�b�g�̊��� (CRLF) ��Ԃ��Ă����߁A`run_ts_trending_feed` / `run_ts_profile_avatar_sync` �� heredoc �₷�ēW���`����ł̌��؂𗧂Ă݂܂��Ƃ��Ɏ��s�ł��邱�Ƃ��m�F���B`.gitattributes` �Ńu���b�g��o���Ă��� Bash �t�@�C���� eol=lf ��w�肵�A`bash -n scripts/test-docker.sh` �ƁA`gh act --workflows .github/workflows/nightly.yml --job trending-feed --container-options "--privileged" -P ubuntu-latest=ghcr.io/catthehacker/ubuntu:act-latest` ��Ńh�J�[ build �܂ł̈ʒm�����������B
    ...
    - メモ (2025年11月16日-5): `gh run view 19405911968 --log` で `Native Test (Linux)` / `Docker Test Suite` の `Run ESLint` が `tests/e2e/helpers/tauriDriver.ts` と `tests/e2e/wdio.desktop.ts` に残っていた `@typescript-eslint/no-dynamic-delete` 向け `eslint-disable` の未使用検知で停止していることを特定。該当ディレクティブを削除したうえで `./scripts/test-docker.ps1 lint` と `gh act --workflows .github/workflows/test.yml --job native-test-linux --container-options "--user root"`（ログ: `tmp/gh_act_native_test.log`）を実行し、Docker / ネイティブ双方の Lint / TypeScript ワークロードが完走することを確認。
