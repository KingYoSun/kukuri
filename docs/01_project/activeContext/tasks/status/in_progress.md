...
- [x] NIPs 準拠イベントスキーマ（NIP-01/10/19/30078 等）の検証/テスト方針を確定する（`iroh-native-dht-plan.md:357`）。
  - 2025年10月30日: Runbook の TODO に移管（Phase 6 で受信バリデーションを整理）。
  - 論点整理
    - [x] 「P2P 受信・イベント生成・オフライン再索引の各層で、NIP 検証責務をどのように分担し、Runbook/Plan のどこに反映するか？」
      ...
      - [x] 実装作業
    - [x] 「NIP-01/10/19 について、許容する marker・relay URL・TLV 長さなどの Pass/Fail 条件をどう定義し、どのドキュメントに確定版を記載するか？」
      ...
      - [x] 実装作業
    - [x] 「kind:30078（アプリ固有イベント）の ["d", …] 必須性や PRE 上書きルールなど、イベントスキーマをどのように規定し、コード/テストへ落とし込むか？」
      ...
      - [x] 実装作業
    - [x] 「NIP-01/10/19/30078 の検証ケースを、ドメインユニット・契約テスト・P2P/Offline 統合テストのどこで扱うか、テストインベントリへどうマッピングするか？」
      ...
      - [x] 実装作業
    - [x] 「NIP-19 と kind30078 用の契約テストを JSON フィクスチャ方式で拡張するのか、それとも別手段を採用するのか？」
      ...
      - [x] 実装作業
    - [x] 「検証失敗時のメトリクス蓄積やログ出力、無効イベントの終端処理（破棄・隔離・保存）をどう定義し、Offline リプレイとの整合をどう取るか？」
...
- 2025年10月30日: GitHub Actions `Native Test (Linux)`/`Format Check` 失敗を調査し、`BootstrapSource` の `Default` 実装を `#[derive(Default)]` + `#[default]` へ置換。`pnpm format` で Prettier 差分を適用し、`gh act -j format-check` / `gh act -j native-test-linux` で成功を確認。`gh act -j docker-test` はローカル Docker on Windows で manifest 解析に失敗するため、`docker compose -f docker-compose.test.yml run --rm test-runner /app/run-tests.sh` で代替検証を実施し正常完了を確認。
