...
- [ ] report/label の永続化テーブルを作る（exp必須、policy_ref/policy_url を紐付け）
- [ ] ルールベースフィルタの定義/優先度/有効無効を管理できるスキーマを作る
- [ ] moderation の処理キュー/進捗/再実行を管理できるようにする

## M4-2 User API: report 受理 / label 参照

- [ ] `POST /v1/reports` を実装する（同意必須 + rate limit + metering（attempts））
- [ ] `GET /v1/labels?target=...` を実装する（topic で絞れる形を検討）

## M4-3 Moderation worker（ルールベース v1）

- [ ] outbox を追従し、対象イベントをルールベースで評価して label を発行する
- [ ] label（39006）は Node Key で署名し、`exp` を必須にする
- [ ] `policy_url`/`policy_ref` を label に含め、透明性を担保する

## M4-4 Admin API/Console: ルール管理と監査

- [ ] ルールの CRUD を Admin API に実装し、更新を監査ログへ残す
- [ ] Admin Console に Moderation 画面を実装する（ルール、通報一覧、手動ラベル）

## M4-5 LLM（v2準備）

- [ ] `LLMProvider` 抽象を用意し、openai/local/disabled を差し替え可能にする（既定: disabled）
- [ ] 送信範囲/保持/予算/停止条件は `llm_moderation_policy.md` に従い、設定の正を `cn_admin` に置く

## M4 完了条件

- [ ] report を受理でき、ルールベースで label が発行され、User API から取得できる
- [ ] 運用者が Admin Console からルールを変更し、監査ログに残る
