...
- [x] Rust テスト: Windows ネイティブ `cargo test`（`kukuri-tauri/src-tauri`）は既知の `STATUS_ENTRYPOINT_NOT_FOUND (0xc0000139)` で停止するため、同ログを記録しつつ `./scripts/test-docker.ps1 rust -NoBuild` を実行して全テストを完走。`cd kukuri-cli && cargo test` はネイティブで成功。
