...
## Community Nodes / `cn-bootstrap` 統合/契約テスト追加

- [x] `refresh_bootstrap_events` の DB 反映を検証する統合テストを `cn-bootstrap` に追加（topic service 行から 39001 が生成されること、および 39000 descriptor が保持されることを確認）。
- [x] `topic_services` 0件時の stale `cn_bootstrap.events(kind=39001)` 削除を検証する統合テストを追加。
- [x] `/healthz` 契約テストを追加し、依存先の応答が `200 -> 503` に遷移したときに `status: ok -> unavailable` と HTTP `200 -> 503` へ遷移することを確認。
- [x] `/metrics` 契約テストを追加し、Prometheus content-type（`text/plain; version=0.0.4`）と payload shape（`cn_up{service="cn-bootstrap"}` を含む）を確認。
- [x] `docs/01_project/activeContext/tasks/priority/community_nodes_roadmap.md` の該当項目を完了化。
- [x] 進捗レポートを `docs/01_project/progressReports/2026-02-09_cn_bootstrap_integration_contract_tests.md` に記録。

## 検証

- [x] `gh act --workflows .github/workflows/test.yml --job community-node-tests --env NPM_CONFIG_PREFIX=/tmp/npm-global`（ログ: `tmp/logs/gh-act-community-node-bootstrap-tests-20260209-225056.log`）
- [x] `gh act --workflows .github/workflows/test.yml --job format-check --env NPM_CONFIG_PREFIX=/tmp/npm-global`（ログ: `tmp/logs/gh-act-format-check-cn-bootstrap-tests-20260209-225413.log`）
- [x] `gh act --workflows .github/workflows/test.yml --job native-test-linux --env NPM_CONFIG_PREFIX=/tmp/npm-global`（ログ: `tmp/logs/gh-act-native-test-linux-cn-bootstrap-tests-20260209-225531.log`）

## Community Nodes / `cn_core::service_config::watch_service_config` LISTEN + poll ハイブリッド反映

- [x] `cn_core::service_config::watch_service_config` に `LISTEN cn_admin_config` を実装し、`pg_notify` を反映経路として利用可能にした。
- [x] listener 切断時の再接続（5秒リトライ）を追加し、poll フォールバックと併用するハイブリッド反映へ拡張した。
- [x] `cn_core::service_config` にテストを追加（通知 payload 判定 / `pg_notify` 即時反映 / 通知なし poll 反映）。
- [x] `docs/01_project/activeContext/tasks/priority/community_nodes_roadmap.md` の該当未実装項目を完了化。
- [x] 進捗レポートを `docs/01_project/progressReports/2026-02-09_cn_core_service_config_watch_listen_hybrid.md` に記録。

## 検証

- [x] `docker build -t kukuri-postgres-age ./kukuri-community-node/docker/postgres-age`（成功）
- [x] `docker run -d --name cn-core-test-postgres-age -e POSTGRES_USER=cn -e POSTGRES_PASSWORD=cn_password -e POSTGRES_DB=cn -p 15432:5432 kukuri-postgres-age -c shared_preload_libraries=age`（成功）
- [x] `docker run --rm -e DATABASE_URL=postgres://cn:cn_password@host.docker.internal:15432/cn -v C:\Users\kgm11\kukuri:/app -w /app/kukuri-community-node rust:1.88-bookworm bash -c "cargo test -p cn-core service_config::tests -- --nocapture"`（成功: 3 passed）
- [x] `gh act --workflows .github/workflows/test.yml --job format-check --env NPM_CONFIG_PREFIX=/tmp/npm-global`（成功。ログ: `tmp/logs/gh-act-format-check-cn-core-config-final-20260210-025214.log`）
- [x] `gh act --workflows .github/workflows/test.yml --job native-test-linux --env NPM_CONFIG_PREFIX=/tmp/npm-global`（成功。ログ: `tmp/logs/gh-act-native-test-linux-cn-core-config-final-20260210-025334.log`）
