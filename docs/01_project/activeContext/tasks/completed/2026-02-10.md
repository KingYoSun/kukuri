...
## `kukuri-cli` 削除移行タスク完了

- [x] `kukuri-cli/` 実体削除と参照移行を完了（`cn-cli` 導線へ統一）
- [x] `rg -n "kukuri-cli" --glob '!docs/**' -S` で非 `docs` 配下の参照 0 件を確認
...
- [x] 必須 `gh act` 3ジョブ完走
  - `format-check` 成功（`tmp/logs/gh-act-format-check-kukuri-cli-removal-20260210-194030.log`）
  - `native-test-linux` 成功（`tmp/logs/gh-act-native-test-linux-kukuri-cli-removal-20260210-194145.log`）
  - `community-node-tests` 成功（`tmp/logs/gh-act-community-node-tests-kukuri-cli-removal-20260210-193617.log`）
- [x] 進捗レポート作成
