...
- [x] `docs/01_project/deprecated/refactoring_plan_2025-08-08_v3.md` の 2.5.2 チェックリストを更新し、アクティブ機能項目を `[x]` 済に変更。`tasks/status/in_progress.md` から当該タスクを削除。
...
- [x] `docs/01_project/deprecated/refactoring_plan_2025-08-08_v3.md` の 2.5.2 チェックリストで未使用機能項目を `[x]` に更新し、`tasks/status/in_progress.md` から当該タスクを削除。
...
- [x] `docs/01_project/deprecated/refactoring_plan_2025-08-08_v3.md` 2.5.2 の「部分的に使用されている機能」チェックを `[x]` に更新し、参照先として新セクションを明記。
...
- [x] `cd kukuri-community-node && cargo test` を同日に実行し、CLI 側 8 件もグリーンであることを確認。  
- [x] 成果を `docs/01_project/deprecated/refactoring_plan_2025-08-08_v3.md`（0.2 節および成功指標チェック）、`docs/01_project/activeContext/artefacts/phase5_ci_path_audit.md`、`tasks/status/in_progress.md` に反映してタスクをクローズ。  
### エビデンス
- `tmp/logs/rust_docker_20251114.log`
- `cn-cli` 直下での `cargo test` 実行ログ（標準出力）
...
- [x] 同日に Rust (`kukuri-tauri/src-tauri/src` + `kukuri-community-node/crates/cn-cli/src`) を計測し、251 ファイル 34,447 行中 4.51%（1,555 行）。重複は `users.rs` / `user_service.rs` / `presentation/commands/*` 等に集中し、cli 側は 22 行のみで目標ラインを十分下回っていることを確認。  
