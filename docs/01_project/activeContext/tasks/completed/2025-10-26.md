...
- 検証: `cargo test`（kukuri-tauri/src-tauri: `linking with cc failed` で停止、Windows 環境の既知事象）／`cargo test`（cn-cli: 成功）。
...
- 検証: `cargo fmt`／`cargo test`（kukuri-tauri/src-tauri）／`cargo test`（cn-cli）。
...
- 検証: `cargo test`（kukuri-tauri/src-tauri）／`cargo test`（cn-cli）。
