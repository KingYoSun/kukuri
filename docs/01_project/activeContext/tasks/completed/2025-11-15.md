...
## 作業名: Offline sync_engine: 再送メトリクスと Runbook/CI 連携
- [x] `kukuri-tauri/src-tauri/src/infrastructure/offline/metrics.rs` を追加し、`metrics::record_outcome` で `OfflineRetryMetricsSnapshot`（成功/失敗・連続失敗・直近 jobId/理由/試行数/backoff/実行時間/タイムスタンプ）を記録。`record_offline_retry_outcome` / `get_offline_retry_metrics` コマンドを `presentation::commands/offline_commands.rs` / `lib.rs` に追加し、フロントから取得できるようにした。  
- [x] `offlineApi` / `useSyncManager` / `SyncStatusIndicator` / `offlineSyncWorker.ts` を更新し、再送ジョブのスケジュール・開始・完了を `retryMetrics` / `scheduledRetry` として UI に表示。BroadcastChannel の `offline-sync:scheduled` / `offline-sync:complete` メッセージ拡張と `record_offline_retry_outcome` 呼び出しで `metrics::record_outcome` と UI が同期するようにした。  
- [x] `scripts/test-docker.{sh,ps1} ts --scenario offline-sync` を分割実行 + JSON レポート保存（`test-results/offline-sync/<timestamp>-*.json`）へ対応。`docs/03_implementation/p2p_mainline_runbook.md` Chapter5/6.4 と `phase5_ci_path_audit.md` に再送メトリクスの triage 手順・artefact 名・ログパスを追記し、`tasks/status/in_progress.md` からタスク13を削除、本ファイルに完了記録を追加。  

### 成果物
- `kukuri-tauri/src-tauri/src/infrastructure/offline/metrics.rs`
- `kukuri-tauri/src-tauri/src/presentation/commands/offline_commands.rs`
- `kukuri-tauri/src-tauri/src/lib.rs`
- `kukuri-tauri/src/api/offline.ts`
- `kukuri-tauri/src/hooks/useSyncManager.ts`
- `kukuri-tauri/src/components/SyncStatusIndicator.tsx`
- `kukuri-tauri/src/serviceWorker/offlineSyncWorker.ts`
- `scripts/test-docker.sh`
- `scripts/test-docker.ps1`
- `docs/03_implementation/p2p_mainline_runbook.md`
- `docs/01_project/activeContext/artefacts/phase5_ci_path_audit.md`
...
## 作業名: mainline::rpc::socket WARN スパム抑制
- [x] `pnpm tauri dev` のログを採取し、`mainline::rpc::socket` ターゲットから `IO error ... (os error 10060)` WARN が 1 秒未満で連続発生していることを確認。`tasks/status/in_progress.md` へタスク24として記録してから原因を切り分けた。
- [x] `kukuri-tauri/src-tauri/src/lib.rs` の `init_logging` で既定ディレクティブを `info,kukuri=debug` に整理しつつ、`RUST_LOG` に `mainline::rpc::socket` が含まれていない場合のみ `mainline::rpc::socket=error` を強制付与するロジックを追加。これにより `RUST_LOG=warn` なホスト環境でも mainline WARN を自動的に抑制し、明示指定時は従来通り出力できるようにした。
- [x] `docs/03_implementation/p2p_mainline_runbook.md` 2.1 に「RUST_LOG 未指定時は mainline WARN が抑制される」旨と再表示するための `$env:RUST_LOG` 例を追記し、Nightly/Runbook から設定意図を参照できるようにした。
- [x] `pnpm tauri dev` を再度起動し、`bootstrap_config`/`iroh_gossip_service` 等の WARN は残しつつ `mainline::rpc::socket` のスパムが出力されないことを確認。取得ログは CLI 上で確認し、本ファイルに記録。
...
## 作業名: Nostr 初期化漏れ修正（新規投稿での EventManager エラー解消）
- [x] `presentation::handlers::event_handler` に `KeyManager` / `EventManagerHandle` 依存を注入し、`initialize_nostr` で現在の鍵を取得して `initialize_with_keypair` を呼び出すフローを実装。アカウント未選択時は Unauthorized を返すよう防御コードを追加。  
- [x] `state.rs` で EventHandler 生成時に鍵/イベントマネージャーを渡すよう配線を更新し、`initialize_nostr` の初期化が必ず鍵情報を受け取るようにした。  
- [x] EventManager 本体 (`infrastructure/event/manager/core.rs`) に `initialize_with_keypair` を追加して `initialize_with_key_manager` から呼び出す形へリファクタリングし、`EventManagerHandle` trait / Gateway テスト実装 / `test_event_service_gateway.rs` を新メソッド対応に更新。  
- [x] `application/shared/tests/event/manager_tests.rs` に KeyPair 直接初期化のユニットテストを追加し、`docs/01_project/activeContext/tasks/status/in_progress.md` から当該タスクを削除して本ファイルに完了記録を残した。  
...
- `kukuri-tauri/src-tauri/src/infrastructure/event/manager/core.rs`
- `kukuri-tauri/src-tauri/src/infrastructure/event/manager_handle.rs`
- `kukuri-tauri/src-tauri/src/presentation/handlers/event_handler.rs`
- `kukuri-tauri/src-tauri/src/state.rs`
- `kukuri-tauri/src-tauri/tests/integration/test_event_service_gateway.rs`

### 検証
- `./scripts/test-docker.ps1 rust`（ログ: `tmp/rust-test.log`）
