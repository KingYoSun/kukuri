...
- [x] `kukuri-tauri/src/hooks/usePosts.ts` の `posts.deleteSuccess` を既存安定キー `posts.deleted` に統一。
- [x] `kukuri-tauri/src/locales/ja.json` / `en.json` / `zh-CN.json` の `common` に `adding` / `conflict` / `count` を追加し、参照キーの未定義を解消。
- [x] 進捗レポート `docs/01_project/progressReports/2026-02-16_issue45_pr1_i18n_key_alignment.md` を追加。

## 検証

- [x] `cd kukuri-tauri && CI=true pnpm install --frozen-lockfile`（pass）
- [x] `cd kukuri-tauri && pnpm vitest run src/tests/unit/hooks/usePosts.test.tsx`（pass）
- [x] `cd kukuri-tauri && pnpm vitest run src/tests/unit/hooks/usePosts.test.tsx src/tests/unit/components/SyncStatusIndicator.test.tsx`（fail: `SyncStatusIndicator` 26件が英語文言前提で失敗。今回の修正対象外の既存テスト期待値不整合）
- [x] `cd kukuri-tauri && pnpm lint`（fail: `src/stores/authStore.ts` の既存未使用変数 `storageError` / `loadError`）
- [x] `cd kukuri-tauri && pnpm eslint src/hooks/usePosts.ts`（pass）
- [x] `cd kukuri-tauri && pnpm prettier --check src/hooks/usePosts.ts src/locales/ja.json src/locales/en.json src/locales/zh-CN.json`（pass）
- [x] `cd /home/kingyosun/kukuri && XDG_CACHE_HOME=/tmp/xdg-cache NPM_CONFIG_PREFIX=/tmp/npm-global DOCKER_CONFIG=/tmp/docker-config gh act --workflows .github/workflows/test.yml --job format-check | tee tmp/logs/gh-act-format-check-issue45-pr1.log`（fail: 既存40ファイルのPrettier未整形）
- [x] `cd /home/kingyosun/kukuri && XDG_CACHE_HOME=/tmp/xdg-cache NPM_CONFIG_PREFIX=/tmp/npm-global DOCKER_CONFIG=/tmp/docker-config gh act --workflows .github/workflows/test.yml --job native-test-linux | tee tmp/logs/gh-act-native-test-linux-issue45-pr1.log`（fail: 既存テストが英語文言/未初期化i18n前提で多数失敗）
- [x] `cd /home/kingyosun/kukuri && XDG_CACHE_HOME=/tmp/xdg-cache NPM_CONFIG_PREFIX=/tmp/npm-global DOCKER_CONFIG=/tmp/docker-config gh act --workflows .github/workflows/test.yml --job community-node-tests | tee tmp/logs/gh-act-community-node-tests-issue45-pr1.log`（pass）

## Issue #45 / PR #49 fix loop（Format Check 失敗解消）

- [x] CI失敗ジョブ（Run `22070578309` / Job `63773790779`）のログを取得し、`Check TypeScript formatting` で `pnpm format:check` が40ファイル未整形として exit 1 になっていることを特定。
- [x] 失敗対象40ファイルはPR差分に含まれるが、PR-1意図（i18nキー整合）とは独立した整形ドリフトのため、最小・安全策として整形のみを適用。
- [x] `kukuri-tauri` の指摘40ファイルに限定して `prettier --write` を適用し、機能ロジック変更なしで Format Check を通過させる状態へ修正。
- [x] 進捗レポート `docs/01_project/progressReports/2026-02-16_issue45_pr49_format_check_fix_loop.md` を追加。

## 検証

- [x] `cd /home/kingyosun/kukuri && gh api -X GET repos/KingYoSun/kukuri/actions/jobs/63773790779/logs > /tmp/job_63773790779_logs.bin`（pass: 失敗ログ取得）
- [x] `cd /home/kingyosun/kukuri/kukuri-tauri && pnpm format:check`（fail: 40ファイル未整形を再現）
- [x] `cd /home/kingyosun/kukuri/kukuri-tauri && xargs -a /tmp/format_warn_files.txt pnpm prettier --write`（pass: 指摘40ファイルのみ整形）
- [x] `cd /home/kingyosun/kukuri/kukuri-tauri && pnpm format:check`（pass）
- [x] `cd /home/kingyosun/kukuri && XDG_CACHE_HOME=/tmp/xdg-cache NPM_CONFIG_PREFIX=/tmp/npm-global DOCKER_CONFIG=/tmp/docker-config ACT_CACHE_DIR=/tmp/act-cache gh act --workflows .github/workflows/test.yml --job format-check 2>&1 | tee tmp/logs/gh-act-format-check-issue45-pr49-fix-loop.log`（pass）
- [x] `cd /home/kingyosun/kukuri && XDG_CACHE_HOME=/tmp/xdg-cache NPM_CONFIG_PREFIX=/tmp/npm-global DOCKER_CONFIG=/tmp/docker-config ACT_CACHE_DIR=/tmp/act-cache gh act --workflows .github/workflows/test.yml --job native-test-linux 2>&1 | tee tmp/logs/gh-act-native-test-linux-issue45-pr49-fix-loop.log`（fail: 既知のi18n文言前提テスト不整合と `date-fns/locale` mock 不備で多数失敗）
- [x] `cd /home/kingyosun/kukuri && XDG_CACHE_HOME=/tmp/xdg-cache NPM_CONFIG_PREFIX=/tmp/npm-global DOCKER_CONFIG=/tmp/docker-config ACT_CACHE_DIR=/tmp/act-cache gh act --workflows .github/workflows/test.yml --job community-node-tests 2>&1 | tee tmp/logs/gh-act-community-node-tests-issue45-pr49-fix-loop.log`（pass）

## Issue #45 / PR #49 fix loop（Native Test + Docker Test Suite 再失敗対応）

- [x] Run `22070890976` の失敗ジョブ（`Native Test (Linux)` / `Docker Test Suite`）ログを再解析し、根因が `date-fns/locale` mock の `enUS` 欠落と、i18n 文言期待値のドリフト連鎖であることを特定。
- [x] PR-1スコープを維持するため、修正は最小差分に限定（`tests/setup.ts` のロケール固定、`OfflineIndicator.test.tsx` mock整理、失敗したテストの i18n キー追従、`authStore.ts` の未使用 catch 変数除去）。
- [x] `Native Test` 相当として `pnpm test` / `pnpm type-check` / `pnpm lint` / `cargo test --locked --workspace --all-features` を再実行し通過を確認。
- [x] `Docker Test Suite` 相当として `./scripts/test-docker.sh all` を実行し、Rust/Clippy/TypeScript/type-check/lint の全ステップ通過を確認。
- [x] 進捗レポート `docs/01_project/progressReports/2026-02-16_issue45_pr49_native_docker_fix_loop.md` を追加。

## 検証

- [x] `gh run view 22070890976 --job 63774951758 --log`（pass: Native失敗ログ取得）
- [x] `gh run view 22070890976 --job 63774971196 --log`（pass: Docker失敗ログ取得）
- [x] `cd kukuri-tauri && pnpm vitest run src/tests/unit/components/OfflineIndicator.test.tsx src/tests/unit/components/P2PDebugPanel.test.tsx src/tests/unit/components/SyncStatusIndicator.test.tsx src/tests/unit/components/auth/WelcomeScreen.test.tsx src/tests/unit/components/posts/PostCard.test.tsx src/tests/unit/components/posts/QuoteForm.test.tsx src/tests/unit/components/topics/TopicCard.test.tsx src/tests/unit/routes/__root.test.tsx src/tests/unit/stores/authStore.accounts.test.ts src/tests/unit/stores/topicStore.test.ts src/tests/integration/ui/multipleAccounts.test.tsx src/tests/integration/ui/relay.integration.test.tsx`（pass）
- [x] `cd kukuri-tauri && pnpm test`（pass）
- [x] `cd kukuri-tauri && pnpm type-check && pnpm lint`（pass）
- [x] `cd kukuri-tauri/src-tauri && cargo test --locked --workspace --all-features`（pass）
- [x] `cd kukuri-tauri/src-tauri && cargo clippy --locked --workspace --all-features -- -D warnings -A dead_code -A unused_variables`（fail: ローカル Rust `1.93.1` で `collapsible_if` が大量検出。CI は Rust `1.86` 固定のため既知差分として記録）
- [x] `HOME=/tmp/kukuri-home DOCKER_CONFIG=/tmp/docker-config timeout --signal=TERM --kill-after=5m 85m ./scripts/test-docker.sh all`（pass）
- [x] `cd /home/kingyosun/kukuri && XDG_CACHE_HOME=/tmp/xdg-cache NPM_CONFIG_PREFIX=/tmp/npm-global DOCKER_CONFIG=/tmp/docker-config ACT_CACHE_DIR=/tmp/act-cache gh act --workflows .github/workflows/test.yml --job format-check 2>&1 | tee tmp/logs/gh-act-format-check-issue45-pr49-native-docker-fix-loop.log`（pass）
- [x] `cd /home/kingyosun/kukuri && XDG_CACHE_HOME=/tmp/xdg-cache NPM_CONFIG_PREFIX=/tmp/npm-global DOCKER_CONFIG=/tmp/docker-config ACT_CACHE_DIR=/tmp/act-cache gh act --workflows .github/workflows/test.yml --job native-test-linux 2>&1 | tee tmp/logs/gh-act-native-test-linux-issue45-pr49-native-docker-fix-loop.log`（pass）
- [x] `cd /home/kingyosun/kukuri && XDG_CACHE_HOME=/tmp/xdg-cache NPM_CONFIG_PREFIX=/tmp/npm-global DOCKER_CONFIG=/tmp/docker-config ACT_CACHE_DIR=/tmp/act-cache gh act --workflows .github/workflows/test.yml --job community-node-tests 2>&1 | tee tmp/logs/gh-act-community-node-tests-issue45-pr49-native-docker-fix-loop.log`（pass）

## Issue #50 GitHub -> Discord webhook 未設定時ガード

- [x] `.github/workflows/github-to-discord.yml` の `notify` job に `DISCORD_WEBHOOK_URL` を job-level `env` として明示し、step 条件判定に使うよう更新。
- [x] `Send to Discord` ステップへ `if: ${{ env.DISCORD_WEBHOOK_URL != '' }}` を追加し、Webhook未設定時に `curl` が実行されないよう修正。
- [x] `Skip Discord POST (webhook not configured)` ステップを追加し、未設定時は理由をログ出力してスキップする挙動を明確化。
- [x] Webhookが設定されている場合の payload 形式（`flags: 4`）および送信処理は維持。
- [x] 進捗レポート `docs/01_project/progressReports/2026-02-16_issue50_github_to_discord_webhook_guard.md` を追加。

## 検証

- [x] `cd /home/kingyosun/kukuri && NPM_CONFIG_PREFIX=/tmp/npm-global XDG_CACHE_HOME=/tmp/xdg-cache DOCKER_CONFIG=/tmp/docker-config ACT_CACHE_DIR=/tmp/act-cache gh act issues -W .github/workflows/github-to-discord.yml -j notify -e tmp/act-events/issues-opened.json 2>&1 | tee tmp/logs/gh-act-github-to-discord-no-webhook.log`（pass: `Skip Discord POST (webhook not configured)` 実行を確認）
- [x] `cd /home/kingyosun/kukuri && NPM_CONFIG_PREFIX=/tmp/npm-global XDG_CACHE_HOME=/tmp/xdg-cache DOCKER_CONFIG=/tmp/docker-config ACT_CACHE_DIR=/tmp/act-cache gh act issues -W .github/workflows/github-to-discord.yml -j notify -e tmp/act-events/issues-opened.json -s DISCORD_WEBHOOK_URL=https://example.com 2>&1 | tee tmp/logs/gh-act-github-to-discord-with-webhook.log`（pass: `Send to Discord` 実行を確認）
- [x] `cd /home/kingyosun/kukuri && NPM_CONFIG_PREFIX=/tmp/npm-global XDG_CACHE_HOME=/tmp/xdg-cache DOCKER_CONFIG=/tmp/docker-config ACT_CACHE_DIR=/tmp/act-cache gh act --workflows .github/workflows/test.yml --job format-check 2>&1 | tee tmp/logs/gh-act-format-check-issue50.log`（pass）
- [x] `cd /home/kingyosun/kukuri && NPM_CONFIG_PREFIX=/tmp/npm-global XDG_CACHE_HOME=/tmp/xdg-cache DOCKER_CONFIG=/tmp/docker-config ACT_CACHE_DIR=/tmp/act-cache gh act --workflows .github/workflows/test.yml --job native-test-linux 2>&1 | tee tmp/logs/gh-act-native-test-linux-issue50.log`（pass）
- [x] `cd /home/kingyosun/kukuri && NPM_CONFIG_PREFIX=/tmp/npm-global XDG_CACHE_HOME=/tmp/xdg-cache DOCKER_CONFIG=/tmp/docker-config ACT_CACHE_DIR=/tmp/act-cache gh act --workflows .github/workflows/test.yml --job community-node-tests 2>&1 | tee tmp/logs/gh-act-community-node-tests-issue50.log`（pass）
