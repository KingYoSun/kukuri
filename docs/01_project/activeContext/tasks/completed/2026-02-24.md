...
## PR #160 review discussion r2844317596 (P2): online復帰時の自動同期をグローバル監視へ移設

- [x] `kukuri-tauri/src/hooks/usePrivacySettingsAutoSync.ts` を新規追加し、未同期プライバシー設定の再送をアプリ常駐で処理するようにした。
- [x] `kukuri-tauri/src/App.tsx` で `usePrivacySettingsAutoSync` を初期化し、設定画面外でも online 復帰同期が動作するようにした。
- [x] `kukuri-tauri/src/routes/settings.tsx` から `online` リスナーと pending 再同期責務を削除し、ページ責務を設定UI操作に限定した。
- [x] `kukuri-tauri/src/tests/unit/hooks/usePrivacySettingsAutoSync.test.tsx` を追加し、online 復帰・ユーザー復元時の同期を検証した。
- [x] `kukuri-tauri/src/tests/unit/routes/settings.test.tsx` と `kukuri-tauri/src/tests/unit/app/App.test.tsx` を更新し、責務分離とフック初期化を検証した。
- [x] 進捗レポート `docs/01_project/progressReports/2026-02-24_pr160_online_recovery_auto_sync_global_listener.md` を追加した。

## 検証（PR #160 / r2844317596）

- [x] `docker compose -f docker-compose.test.yml run --rm --build ts-test pnpm vitest run src/tests/unit/routes/settings.test.tsx src/tests/unit/app/App.test.tsx src/tests/unit/hooks/usePrivacySettingsAutoSync.test.tsx`（pass）
- [x] `NPM_CONFIG_PREFIX=/tmp/npm-global gh act --workflows .github/workflows/test.yml --job format-check`（pass）
- [x] `NPM_CONFIG_PREFIX=/tmp/npm-global gh act --workflows .github/workflows/test.yml --job native-test-linux`（pass）
- [x] `NPM_CONFIG_PREFIX=/tmp/npm-global gh act --workflows .github/workflows/test.yml --job community-node-tests`（pass）

## Issue #156: Tauriヘッダー接続表示の簡素化（Wifi時刻表示削除、アンテナ色切替）

- [x] `kukuri-tauri/src/components/layout/Header.tsx` から `RealtimeIndicator` 表示を除去し、接続表示をアンテナボタンに一本化した。
- [x] `kukuri-tauri/src/components/layout/AntennaStatusDialog.tsx` に `navigator.onLine` と `online/offline` イベント監視を追加し、アンテナアイコン色をオンライン時 `text-green-500` / オフライン時 `text-red-500` に切り替えるようにした。
- [x] `kukuri-tauri/src/tests/unit/components/layout/Header.test.tsx` を更新し、Wifi時刻表示文言の不在とアンテナ色切替（緑→赤）を検証するテストを追加した。

## 検証（Issue #156）

- [x] `docker compose -f docker-compose.test.yml build test-runner ts-test`（pass）
- [x] `docker compose -f docker-compose.test.yml run --rm ts-test pnpm vitest run src/tests/unit/components/layout/Header.test.tsx`（pass）
- [x] `docker compose -f docker-compose.test.yml run --rm rust-test`（pass）

## Issue #157: localhost Bootstrap 自動適用と Tauri hostname 解決の修正

- [x] `kukuri-tauri/src-tauri/src/infrastructure/p2p/bootstrap_config.rs` に socket address 解決ヘルパーを追加し、`localhost` を `127.0.0.1` と同等扱いにした。
- [x] bootstrap descriptor/設定値/入力値の各経路で hostname 付きアドレスを共通解決し、`localhost` 混在時の重複・未適用を防止した。
- [x] `kukuri-tauri/src-tauri/src/infrastructure/p2p/utils.rs` の `parse_node_addr` を hostname 解決対応に更新し、Tauri クライアントの node address 解析を修正した。
- [x] `kukuri-tauri/src-tauri/src/infrastructure/p2p/iroh_network_service.rs` の peer 追加処理を `parse_node_addr` 共通ロジックへ統一した。
- [x] `kukuri-tauri/src-tauri/src/presentation/handlers/community_node_handler.rs` に `localhost` descriptor を loopback 正規化するテストを追加した。
- [x] 進捗レポート `docs/01_project/progressReports/2026-02-24_issue157_localhost_bootstrap_auto_apply.md` を追加した。

## 検証（Issue #157）

- [x] `cd kukuri-tauri/src-tauri && cargo test set_config_treats_localhost_bootstrap_nodes_as_loopback -- --nocapture`（pass）
- [x] `cd kukuri-tauri/src-tauri && cargo test test_parse_node_addr_localhost -- --nocapture`（pass）
- [x] `cd kukuri-tauri/src-tauri && cargo test sanitize_bootstrap_nodes_rewrites_unspecified_addresses -- --nocapture`（pass）
- [x] `cd kukuri-tauri/src-tauri && cargo test`（pass）
- [x] `cd kukuri-tauri/src-tauri && cargo clippy -- -D warnings`（pass）
- [x] `docker compose -f docker-compose.test.yml up -d community-node-postgres`（pass）
- [x] `docker compose -f docker-compose.test.yml build test-runner`（pass）
