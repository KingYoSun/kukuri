...
- 検証: `cargo fmt`／`cargo test`（kukuri-tauri/src-tauri は Windows 既知の `STATUS_ENTRYPOINT_NOT_FOUND` で停止するため `./scripts/test-docker.ps1 rust` で代替実行）／`cargo test`（cn-cli）。
...
- 検証: `cargo fmt`／`cargo test`（kukuri-tauri/src-tauri は `./scripts/test-docker.ps1 rust` で完走、Windows ネイティブ実行は既知エラー）／`cargo test`（cn-cli）。
...
- 検証: `cargo fmt`（kukuri-tauri/src-tauri）／`cargo test`（kukuri-tauri/src-tauri、約6分で完走）／`cargo test`（cn-cli）。
...
- 検証: `cargo fmt`／`./scripts/test-docker.sh rust`（kukuri-tauri/src-tauri の全テスト）／`cargo test`（cn-cli）。
...
- 検証: `./scripts/test-docker.sh rust`（kukuri-tauri/src-tauri）で regression なし、`cargo test`（cn-cli）も成功。
...
- 検証: `cargo fmt`／`cargo test`（kukuri-tauri/src-tauri、Windows 既知エラーのため Docker テストにフォールバック予定）／`cargo test`（cn-cli）。
...
- 検証: `cargo fmt`（kukuri-tauri/src-tauri）／`cargo test`（kukuri-tauri/src-tauri は Docker 経由で完走、Windows ネイティブは `STATUS_ENTRYPOINT_NOT_FOUND` を確認）／`cargo test`（cn-cli）。

## Phase 5 EventManager インフラ移行（EVT-MGR-04）
- `modules::event` 配下の `EventManager` / `EventHandler` / `nostr_client` を削除し、`infrastructure::event::{manager,handler,nostr_client_manager}` として再配置。`EventManagerHandle`・Gateway・`application/shared/tests/event/manager_tests.rs` を新パスへ更新し、`modules::mod.rs` から Legacy イベントモジュールの再エクスポートを撤去した。
- `infrastructure::event/handler.rs` の SQLx クエリを最新スキーマ（`followed_pubkey` / `reactor_pubkey` など）に合わせて修正し、`cargo sqlx prepare` で `.sqlx/query-*.json` を再生成。`phase5_dependency_inventory_template.md` と `docs/01_project/deprecated/refactoring_plan_2025-08-08_v3.md` の EventManager 行を 2025年10月27日完了として更新し、`tasks/status/in_progress.md` に移行ログを追記。
- 検証: `SQLX_OFFLINE=true cargo test --workspace --all-features`（kukuri-tauri/src-tauri、約10分）／`cargo test`（cn-cli、7.7秒）。SQLx の離線キャッシュ更新後に各テストがグリーンで完了することを確認。
