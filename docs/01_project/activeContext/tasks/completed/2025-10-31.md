# 2025年10月31日 完了タスク（リファクタリング計画関連)

最終更新日: 2025年10月31日

このエントリは 2025年10月31日時点で in_progress.md に記載されていた完了済み項目と補足メモを移設したものです。履歴確認のため、元の構成・チェックボックスを保持しています。
...
- [x] `application/services/p2p_service.rs` を `core` / `builder` / `status` モジュールへ分割し、既存テストを `tests.rs` に再配置。DI とメトリクス参照を維持したまま、Mock 実装を再利用しやすい構成へ整理（2025年10月31日）。
- [x] `domain/entities/event.rs` を `model` / `kind` / `validation` サブモジュールへ再構成し、NIP/BOLT 系バリデーションと TLV ヘルパーをモジュール単位で管理。NIP ごとのテストを `tests/nip01.rs` などに分割し、Kind30078 スキーマ検証のカバレッジを維持（2025年10月31日）。
- [x] ドキュメント (`refactoring_plan_2025-08-08_v3.md`) とタスク進捗を更新し、再構成済みモジュールとテストレイアウトをアーキテクチャ改善の進捗に反映（2025年10月31日）。
...
- [x] Phase 5 Workstream A: Rustモジュール再構成（`docs/01_project/deprecated/refactoring_plan_2025-08-08_v3.md`）
...
- [x] Phase 5 Workstream B: Rustカバレッジ計測（`cargo tarpaulin`）の定着（参照: `docs/01_project/deprecated/refactoring_plan_2025-08-08_v3.md` 369-375行）
...
- [x] Phase 5 Legacy KeyManager 移行（参照: `docs/01_project/deprecated/refactoring_plan_2025-08-08_v3.md` 343行／`docs/01_project/activeContext/artefacts/phase5_dependency_inventory_template.md` 28行）
...
- [x] Phase 5 Legacy SecureStorage デバッグユーティリティ整理（参照: `docs/01_project/deprecated/refactoring_plan_2025-08-08_v3.md` 342行／`docs/01_project/activeContext/artefacts/phase5_dependency_inventory_template.md` 31行）
  ...
  - 2025年10月25日: `modules/secure_storage`（mod/tests）を削除し、`refactoring_plan_2025-08-08_v3.md` / `phase5_dependency_inventory_template.md` に完了状況を記録。Windows `cargo test` は `STATUS_ENTRYPOINT_NOT_FOUND`（既知）で停止したが、`./scripts/test-docker.ps1 rust` / `pnpm test` / `cn-cli cargo test` で回帰を確認。
- [x] Phase 5 BookmarkManager アーカイブ（参照: `docs/01_project/deprecated/refactoring_plan_2025-08-08_v3.md` 341行／`docs/01_project/activeContext/artefacts/phase5_dependency_inventory_template.md` 30行）
  ...
  - 2025年10月26日: `modules::bookmark` ディレクトリを削除し、`phase5_dependency_inventory_template.md`／`refactoring_plan_2025-08-08_v3.md`／`tauri_app_implementation_plan.md`／`nostr_reactions_implementation.md` を更新。`cargo test`（kukuri-tauri/src-tauri, cn-cli）で回帰確認。
...
- 2025年10月17日: `ApplicationContainer` を導入し、Base64 永続化した iroh シークレットキーからノード ID を再利用する初期化と、`NetworkConfig.bootstrap_peers` を `IrohNetworkService` 初期化時に適用する仕組みを整備。Docker 経由の `cargo test` と `cn-cli` のテストまで確認済み。
