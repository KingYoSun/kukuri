...
## Community Node 39000/39001 Gossip/DHT 統合フロー

- [x] 39000/39001 の P2P 受信イベントをクライアント側で取り込み、キャッシュへ反映する経路を追加
- [x] bootstrap nodes/services の取得でキャッシュと HTTP 再取得（`next_refresh_at`/stale 判定）を統合
- [x] 39000/39001 の署名/exp 検証と addressable 置換ルールでのキャッシュ整理を実装
- [x] `CommunityNodeHandler` に bootstrap キャッシュのユニットテストを追加
- [x] 進捗レポートを作成（`docs/01_project/progressReports/2026-02-03_community_node_bootstrap_gossip_cache.md`）
...
- [x] 39022 を kind 一覧に反映し、community_node_plan / KIP-0001 の一覧を整合
- [x] Topic ID 形式を kukuri:<64hex> / kukuri:global に固定し、未決事項から除外
- [x] NIP-44 実装方針を v2 前提に統一（nostr-sdk nip44::Version::V2、key.envelope は必須）
- [x] P2P-only と Access Control の運用範囲（Node HTTP API では扱わない／membership はローカル判断）を明文化
- [x] 進捗レポート: docs/01_project/progressReports/2026-02-03_community_node_spec_alignment.md
...
- [x] `./scripts/test-docker.ps1 rust`（39000/39001 Gossip/DHT キャッシュ統合後の再実行）
