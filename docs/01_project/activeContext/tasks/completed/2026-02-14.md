...
## Issue #5: Community Node テストコマンドの標準化（ドキュメント整備）

- [x] `README.md` / `README.ja.md` の community-node テスト導線を「全OSでコンテナ既定」に統一
- [x] `docs/03_implementation/docker_test_environment.md` に全OS共通の community-node テスト/ビルドコマンドを追加
- [x] `docs/03_implementation/community_nodes/ops_runbook.md` / `docs/03_implementation/ci_required_checks_policy.md` の運用方針を container-first に更新
- [x] `AGENTS.md` と `tasks/status/in_progress.md`、`tasks/priority/community_nodes_roadmap.md` の記述を同方針に同期
- [x] 進捗レポート `docs/01_project/progressReports/2026-02-14_issue5_community_node_test_command_standardization.md` を作成

## 検証（Issue #5 ドキュメント整備）

- [x] `python3 scripts/check_date_format.py`（成功）
- [x] `NPM_CONFIG_PREFIX=/tmp/npm-global gh act --workflows .github/workflows/test.yml --job format-check --action-cache-path /tmp/act-cache --cache-server-path /tmp/actcache --artifact-server-path /tmp/act-artifacts`（成功、ログ: `tmp/logs/gh-act-format-check-issue5-20260214.log`）
- [x] `NPM_CONFIG_PREFIX=/tmp/npm-global gh act --workflows .github/workflows/test.yml --job native-test-linux --action-cache-path /tmp/act-cache --cache-server-path /tmp/actcache --artifact-server-path /tmp/act-artifacts`（成功、ログ: `tmp/logs/gh-act-native-test-linux-issue5-20260214.log`）
- [x] `NPM_CONFIG_PREFIX=/tmp/npm-global gh act --workflows .github/workflows/test.yml --job community-node-tests --action-cache-path /tmp/act-cache --cache-server-path /tmp/actcache --artifact-server-path /tmp/act-artifacts`
  - 初回失敗（`cn-admin-api` 契約テストで `tuple concurrently updated`）: `tmp/logs/gh-act-community-node-tests-issue5-20260214.log`
  - 再実行成功: `tmp/logs/gh-act-community-node-tests-issue5-20260214-rerun.log`
