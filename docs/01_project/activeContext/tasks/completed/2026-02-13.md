...
## Community Nodes / `cn-user-api` bootstrap `ETag` をレスポンスハッシュ方式へ変更

- [x] `cn-user-api` の `bootstrap::respond_with_events` で `ETag` 生成を `updated_at+件数` 方式から、レスポンスボディ（`items` + `next_refresh_at`）の BLAKE3 ハッシュ方式へ変更
- [x] 契約テスト `bootstrap_services_etag_changes_when_body_changes_with_same_count_and_second` を追加し、「件数不変・同秒更新でも `ETag` が変化する」ことを固定
- [x] `community_nodes_roadmap.md` の該当未実装項目を `[x]` に更新
- [x] 進捗レポート `docs/01_project/progressReports/2026-02-13_cn_user_api_bootstrap_etag_hash_contract.md` を作成

## 検証（`cn-user-api` bootstrap `ETag` ハッシュ契約）

- [x] `./scripts/test-docker.ps1 rust -NoBuild`（ログ上成功、終了コード `-1` は既知）
  - ログ: `tmp/logs/test-docker-rust-cn-user-api-bootstrap-etag-20260213-transcript.log`
- [x] `docker run --rm --network kukuri_community-node-network -e DATABASE_URL=postgres://cn:cn_password@community-node-postgres:5432/cn -v ${PWD}:/workspace -w /workspace/kukuri-community-node kukuri-test-runner bash -lc "source /usr/local/cargo/env && cargo test -p cn-user-api bootstrap_services_ -- --nocapture --test-threads=1"`（成功）
  - ログ: `tmp/logs/cn-user-api-bootstrap-services-contract-20260213.log`
- [x] `docker run --rm --network kukuri_community-node-network -e DATABASE_URL=postgres://cn:cn_password@community-node-postgres:5432/cn -v ${PWD}:/workspace -w /workspace/kukuri-community-node kukuri-test-runner bash -lc "source /usr/local/cargo/env && cargo test --workspace --all-features && cargo build --release -p cn-cli"`（成功）
  - ログ: `tmp/logs/community-node-rust-workspace-cn-user-api-bootstrap-etag-20260213.log`
- [x] `$env:NPM_CONFIG_PREFIX='/tmp/npm-global'; gh act --workflows .github/workflows/test.yml --job format-check`（初回は rustfmt 差分で失敗、再実行成功）
  - 失敗ログ: `tmp/logs/gh-act-format-check-cn-user-api-bootstrap-etag-20260213.log`
  - 成功ログ: `tmp/logs/gh-act-format-check-cn-user-api-bootstrap-etag-20260213-rerun.log`
- [x] `$env:NPM_CONFIG_PREFIX='/tmp/npm-global'; gh act --workflows .github/workflows/test.yml --job native-test-linux`（成功、既知の `useRouter` 警告のみ）
  - ログ: `tmp/logs/gh-act-native-test-linux-cn-user-api-bootstrap-etag-20260213.log`
- [x] `$env:NPM_CONFIG_PREFIX='/tmp/npm-global'; gh act --workflows .github/workflows/test.yml --job community-node-tests`（成功）
  - ログ: `tmp/logs/gh-act-community-node-tests-cn-user-api-bootstrap-etag-20260213.log`

## Community Nodes / `cn-user-api` bootstrap 認証必須時の `428 CONSENT_REQUIRED` 契約境界を追加

- [x] `cn-user-api` 契約テストに `bootstrap_nodes_contract_requires_consent_when_authenticated` を追加し、`GET /v1/bootstrap/nodes` で「認証済みだが未同意」の `428 CONSENT_REQUIRED` を固定
- [x] `cn-user-api` 契約テストに `bootstrap_services_contract_requires_consent_when_authenticated` を追加し、`GET /v1/bootstrap/topics/{topic_id}/services` で同契約を固定
- [x] 既存の `401 + WWW-Authenticate` テストとの境界を固定するため、`428` 応答で `WWW-Authenticate` を返さないことを明示検証
- [x] `community_nodes_roadmap.md` の該当未実装項目を `[x]` に更新
- [x] 進捗レポート `docs/01_project/progressReports/2026-02-13_cn_user_api_bootstrap_consent_required_contract_boundary.md` を作成

## 検証（`cn-user-api` bootstrap `428 CONSENT_REQUIRED` 契約境界）

- [x] `./scripts/test-docker.ps1 rust`（ログ上成功、終了コード `-1` は既知）
  - ログ: `tmp/logs/test-docker-rust-cn-user-api-bootstrap-consent-20260213.log`
- [x] `docker run --rm --network kukuri_community-node-network -e DATABASE_URL=postgres://cn:cn_password@community-node-postgres:5432/cn -v ${PWD}:/workspace -w /workspace/kukuri-community-node kukuri-test-runner bash -lc "source /usr/local/cargo/env && cargo test -p cn-user-api bootstrap_nodes_contract_requires_consent_when_authenticated -- --nocapture --test-threads=1 && cargo test -p cn-user-api bootstrap_services_contract_requires_consent_when_authenticated -- --nocapture --test-threads=1"`（成功）
  - ログ: `tmp/logs/cn-user-api-bootstrap-consent-contract-20260213.log`
- [x] `$env:NPM_CONFIG_PREFIX='/tmp/npm-global'; gh act --workflows .github/workflows/test.yml --job format-check`（初回は rustfmt 差分で失敗、再実行成功）
  - 失敗ログ: `tmp/logs/gh-act-format-check-cn-user-api-bootstrap-consent-20260213.log`
  - 成功ログ: `tmp/logs/gh-act-format-check-cn-user-api-bootstrap-consent-20260213-rerun.log`
- [x] `$env:NPM_CONFIG_PREFIX='/tmp/npm-global'; gh act --workflows .github/workflows/test.yml --job native-test-linux`（成功、既知の `useRouter` 警告のみ）
  - ログ: `tmp/logs/gh-act-native-test-linux-cn-user-api-bootstrap-consent-20260213.log`
- [x] `$env:NPM_CONFIG_PREFIX='/tmp/npm-global'; gh act --workflows .github/workflows/test.yml --job community-node-tests`（成功）
  - ログ: `tmp/logs/gh-act-community-node-tests-cn-user-api-bootstrap-consent-20260213.log`

## Community Nodes / `cn-cli` 統合スモークテスト追加と daemon コマンド仕様整合

- [x] `cn-cli` 統合テスト（`cli_access_control_node_key_integration.rs`）に、`migrate` / `config seed` / `admin bootstrap|reset-password` / `openapi export` / `p2p` 系を横断するスモークテストを追加
- [x] スモーク内で `p2p node-id` の決定論的出力（同一 secret key で同一 node id）と、`cn bootstrap` / `cn relay` ヘルプ契約を固定
- [x] `cn_cli_migration.md` を更新し、現行仕様を `cn bootstrap` / `cn relay`（daemon サブコマンドなし）として明記
- [x] `community_nodes_roadmap.md` の該当未実装項目を `[x]` に更新
- [x] 進捗レポート `docs/01_project/progressReports/2026-02-13_cn_cli_smoke_tests_and_command_alignment.md` を作成

## 検証（`cn-cli` 統合スモーク + コマンド仕様整合）
...
- [x] `$env:NPM_CONFIG_PREFIX='/tmp/npm-global'; gh act --workflows .github/workflows/test.yml --job format-check`（初回は `rustfmt` 差分で失敗、再実行成功）
  - 失敗ログ: `tmp/logs/gh-act-format-check-cn-cli-smoke-20260213.log`
  - 成功ログ: `tmp/logs/gh-act-format-check-cn-cli-smoke-rerun-20260213.log`
- [x] `$env:NPM_CONFIG_PREFIX='/tmp/npm-global'; gh act --workflows .github/workflows/test.yml --job native-test-linux`（成功、既知の `useRouter` 警告のみ）
