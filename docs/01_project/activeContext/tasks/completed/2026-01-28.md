...
## GitHub Actions 失敗修正（AccessControlService dead_code）

- [x] join.request / invite payload の未使用フィールドを検証に取り込み、dead_code を解消
- [x] invite nonce mismatch / requested_at 不正のテストを追加
- [x] `./scripts/test-docker.ps1 rust` を再実行して通過

### 検証

- `./scripts/test-docker.ps1 rust`
- `gh act --workflows .github/workflows/test.yml --job format-check`（警告: git clone の `some refs were not updated` / pnpm `approve-builds` 警告）
- `gh act --workflows .github/workflows/test.yml --job native-test-linux`（警告: git clone の `some refs were not updated` / React `act(...)` 警告 / `useRouter` が RouterProvider 外で使用の警告）

## Community Node Access Control P2P-only invite/join/key.envelope E2E/統合テスト

- [x] P2P-only の invite→join.request→key.envelope を AccessControlService 2インスタンスで再現し、invite scope 暗号化投稿まで検証
- [x] TestKeyManager/TestGroupKeyStore/TestGossipService で join.request / key.envelope の手動中継を確認
- [x] PostService で暗号化 payload を生成し、EncryptedPostPayload の scope/epoch を確認

### 検証

- `./scripts/test-docker.ps1 rust`
- `gh act --workflows .github/workflows/test.yml --job format-check`・警告: git clone の `some refs were not updated` / pnpm `approve-builds` notice
- `gh act --workflows .github/workflows/test.yml --job native-test-linux`・警告: git clone の `some refs were not updated` / React `act(...)` 警告 / `useRouter` が RouterProvider 外の警告
