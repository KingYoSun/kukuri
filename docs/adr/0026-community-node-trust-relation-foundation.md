...
- read: trust は **絶対成分（viewer 非依存）+ 相対成分（viewer / cluster 依存）**の合成として返す。いずれも断定ラベル（「このユーザーは troll」）ではなく根拠つき advisory（basis / 寄与 signal / confidence / visibility / expiry を説明可能、trust-semantics §4）。
...
- relation は index の co-participation を入力にするため、indexing（#404 / ADR 0025）に依存する。最小構成は Postgres + 解析 worker、scale は NebulaGraph（§6.1）。
...
### 6.1 relation clustering の最小構成と scale path
- **最小構成**: **Postgres を正本**とし、**項目ごとの解析 worker** が複数項目を点数化する（例: 共有 topic 数、friend-of-friend、co-participation 頻度、follow projection）。pairwise proximity はこれら項目スコアの合成として算出する。
- **scale path**: profile に応じて **NebulaGraph** 等のグラフ DB を導入し大規模化に対応する。
- **未決（要判断）**: 相対指標が relation（graph proximity）計算を要するため、**最小構成から NebulaGraph を導入するか、Postgres + worker から始めて後で差し替えるか**。後者を採る場合も **graph-store 抽象境界**を置き、NebulaGraph を後から差し込めるようにする。

### 6.2 trust scoring（絶対 / 相対の合成）
...
- 合成方法・重み・閾値・decay / appeal（`AppealStatus`）反映の具体は未決。
