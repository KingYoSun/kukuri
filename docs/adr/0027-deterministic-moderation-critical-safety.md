...
本 ADR のスコープは **決定論的検知（`Basis::KnownHashMatch` / `Basis::ProviderVerdict` = confirmed）**と、その上に載る **共通の verdict / fail-closed / signed event / visibility / provider abstraction フレームワーク**である。classifier ベースの **非決定論的検知（`Basis::ClassifierScore` = suspected / VLM）**の詳細は #411 に委ね、本 ADR は両者が差さる共通枠組みと決定論側を固定する。
...
- 必須 contract（大半は #353 / #405 で実装済み。本 ADR で normative に固定）:
  - `deterministic_confirmed_requires_known_hash_or_provider_verdict`
...
- **決定論的 confirmed の根拠は `Basis::KnownHashMatch` / `Basis::ProviderVerdict` に限る**。`Basis::ClassifierScore` は suspected どまり（confirmed に昇格させない）。
...
- critical route（CSAM / CSE / grooming）と general route（nsfw / violence / hate / harassment / spam / malware / phishing）を分離する（`SafetyCategory::is_critical_safety`）。同一 queue / reason code にまとめない。
...
- event の `ModerationAction` は `Exclude` / `Hold` / `Quarantine` / `RiskLabel`。
...
- confirmed（決定論）CSAM の risk signal は ADR 0026 の **trust 絶対成分**の入力になる（relation 非依存、report-bomb 不動）。#406 が runtime で反映を結線する。
...
### 2.8 reporting / appeal / audit
- 通報は中央集約しない。実際に surfacing / moderation / cache / report に関与した node の authority scope へ route する（issuer node を report target 候補に含める）。
- appeal は `AppealStatus`（None / Disputed / Cleared）で表現。operator は有害コンテンツを再配布せずに exclude を説明・監査できる。

### 2.9 readiness before public indexing
- known-CSAM provider 設定 / credential 検証 / `index_before_scan=false` / `on_scan_error` が fail-closed / signed event 有効 / permanent blob storage 無効 / scan coverage metrics（critical-safety.md §11、`cn-operator safety readiness`）。
...
- **ADR 0026**: confirmed（決定論）= trust 絶対成分の入力。
...
- `NoKnownMatch` を safe の証明にしない。confirmed は known-hash / provider-verdict に限る。
