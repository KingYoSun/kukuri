...
`list_dome_connection_topology`は購読を開始せず、許可済みreplicaの情報からlocal projectionを更新する。private channelは既存stateの権限検査を通してからreplicaへアクセスする。mutationでも同じ検査を購読開始より前に行う。署名・docs・hintの接続書込は既存の明示actionと正当なlifecycle処理に限定する。
