...
| 利用者が保存・同意した Community Node | manifest／policy 取得、認証、consent、bootstrap／rendezvous、検索・発見、明示した indexing と索引状況の確認、成人向け表現の推定の照会、通報、tester feedback 等 | 選択した Node capability の提供 | 公開鍵、proof、endpoint 情報、topic rendezvous key、検索 query、対象識別子（索引状況の確認では自分の申請対象と確認したい topic／channel。非公開チャンネルの索引対象判定は明示確認後の所属証明を伴います）、成人向け表現の推定の照会では表示した投稿の ID と添付ファイルの識別子（推定の採用を選んだ Node だけ。本文・閲覧履歴・social graph は含みません）、通報／feedback の入力項目等。機能ごとに異なります | 当該 Node の privacy／外部送信／retention 文書に従います。Node 運営者と kukuri 運営者が同一とは限りません |
