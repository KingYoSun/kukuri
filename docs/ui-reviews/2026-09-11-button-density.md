...
- Summary: textボタンを全丸のまま高さ2rem（`sm`は1.75rem）、横padding 0.625rem（`sm`は0.5rem）へ縮小。icon-only controlは2rem前後、投稿カードのavatarは1.75rem、profile overviewのavatarは3remへ縮小し、avatarは大きさに関わらず常に`--radius-pill`（全丸）に固定。投稿カードの余白と投稿meta／actionsの間隔を1段詰める。文字サイズは変更しない。Control Centerの「場所」topic行はtopic名だけで1行を確保し、操作列を次の行へ置く。文言「プライベートチャンネルを作成または参加」を「チャンネル作成・参加」へ変更。Control Center見出しの「全体ナビゲーション」（eyebrow）は意味を持たないため削除。
