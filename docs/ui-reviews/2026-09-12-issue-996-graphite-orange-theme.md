...
- Superseded by: None
- PR: Issue #996（PR番号と最終検証は下記作業記録へ集約）
...
- Conditions: browserは日本語、dark／light、1600×1000／390×844、scale factor 1。Timeline＋Profile＋Explore、表示設定、投稿Dialogを同条件比較。narrowは既存の1 Column＝1 viewportを維持。各OSのnative条件は作業記録を参照。
...
- Validation / Not verified / Review result: [作業記録](../progress/2026-09-12-996-graphite-orange-theme.md)に実行結果と未確認事項を集約する。配色の採用は承認済み。実装の完了判定は同記録の最終validationに従う。
