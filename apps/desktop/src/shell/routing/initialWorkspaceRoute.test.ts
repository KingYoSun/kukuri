...
const PRIVATE_SCOPE = { topicId: 'kukuri:topic:demo', channelId: 'channel-1' };
...
describe('initialHashForRestoredWorkspace', () => {
  it('maps each Column kind to its canonical target', () => {
    ...
    ).toBe('#/timeline?topic=kukuri%3Atopic%3Ademo&channel=channel-1');
    expect(
      initialHashForRestoredWorkspace(
        workspaceWith({ id: 'c', kind: 'thread', entityId: 'post-1' })
      )
    ).toBe('#/timeline?topic=kukuri%3Atopic%3Ademo&context=thread&threadId=post-1');
    expect(
      initialHashForRestoredWorkspace(
        workspaceWith({ id: 'c', kind: 'profile', entityId: 'a'.repeat(64) })
      )
    ).toBe(`#/timeline?topic=kukuri%3Atopic%3Ademo&context=author&authorPubkey=${'a'.repeat(64)}`);
    expect(
      initialHashForRestoredWorkspace(workspaceWith({ id: 'c', kind: 'profile' }))
    ).toBe('#/profile?topic=kukuri%3Atopic%3Ademo');
    expect(
      initialHashForRestoredWorkspace(workspaceWith({ id: 'c', kind: 'notifications' }))
    ).toBe('#/notifications?topic=kukuri%3Atopic%3Ademo');
    expect(
      initialHashForRestoredWorkspace(workspaceWith({ id: 'c', kind: 'messages' }))
    ).toBe('#/messages?topic=kukuri%3Atopic%3Ademo');
    expect(
      initialHashForRestoredWorkspace(
        workspaceWith({ id: 'c', kind: 'conversation', entityId: 'b'.repeat(64) })
      )
    ).toBe(`#/messages?topic=kukuri%3Atopic%3Ademo&peerPubkey=${'b'.repeat(64)}`);
    expect(
      initialHashForRestoredWorkspace(workspaceWith({ id: 'c', kind: 'explore' }))
    ).toBe('#/explore?topic=kukuri%3Atopic%3Ademo');
    expect(
      initialHashForRestoredWorkspace(
        workspaceWith({ id: 'c', kind: 'stream', entityId: 'session-1' })
      )
    ).toBe('#/live?topic=kukuri%3Atopic%3Ademo&sessionId=session-1');
    expect(
      initialHashForRestoredWorkspace(
        workspaceWith({ id: 'c', kind: 'metaverse', entityId: 'room-1' })
      )
    ).toBe('#/game?topic=kukuri%3Atopic%3Ademo&roomId=room-1');
  });
