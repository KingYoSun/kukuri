...
test('initial consent failure stays in terms, interruption returns to explanation, success opens profile', async () => {
  ...
  await waitFor(() => expect(within(terms).getByRole('button', { name: 'Accept', exact: true })).toBeEnabled());
  await user.click(within(terms).getByRole('button', { name: 'Accept', exact: true }));
