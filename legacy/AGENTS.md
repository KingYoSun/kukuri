...
## Rebuild Track（2026年03月10日）
- 再構築タスクは `next/` を主対象とし、`kukuri-tauri/` と `kukuri-community-node/` は cutover まで legacy 参照として扱う。
- Linux-first MVP の実行入口は root から `cargo xtask doctor|check|test|e2e-smoke` に統一する。
- `next/apps/desktop` の依存固定と実行は `npx pnpm@10.16.1 ...` を使用する。`corepack/pnpm` が壊れていてもこの経路を優先する。
- MVP 中は `next/` 側へ Windows 対応、DHT discovery、community-node 連携を実装しない。
