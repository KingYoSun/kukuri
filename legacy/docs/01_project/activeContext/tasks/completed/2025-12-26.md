...
- cn-cli は nostr-sdk 依存が無いため変更なし（追加が必要なら別途対応）。
...
## cn-cli / kukuri-tauri 依存ライブラリ更新（Cargo.toml）

### 概要
cn-cli と kukuri-tauri の依存ライブラリを最新安定版へ更新し、API 変更に合わせて Rust 実装と CI の Rust バージョンを更新しました。

### 対応内容
- `kukuri-tauri/src-tauri/Cargo.toml` / `kukuri-community-node/crates/cn-cli/Cargo.toml` の依存を最新安定版へ更新（`bincode` は 3.0.0 が `compile_error!` のため 2.0.1 を維持）。
