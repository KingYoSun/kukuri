## リファクタリング計画アーカイブ（Task 26）

- [x] `docs/01_project/refactoring_plan_2025-08-08_v3.md` を `docs/01_project/deprecated/` 以下へ移動し、本文に完了ステータスと参照先を追記。旧パス側には stub を配置し、利用者がアーカイブ先へ誘導されるようにした。
- [x] `docs/01_project/deprecated/README.md` を新設し、アーカイブ対象ドキュメントの一覧・ステータス・最新参照先を整理した。
- [x] `roadmap.md` / `design_doc.md` / `tauri_app_implementation_plan.md` / `phase5_user_flow_summary.md` など主要な参照元を更新し、「アーカイブ済み」ラベルと新パスを追記。`phase5_*` artefact や `tasks/completed/*` のリンクも一括で `docs/01_project/deprecated/` 配下へ差し替えた。
...
## `/search` 補助検索 artefact の Nightly 固定化（Task 33）

- [x] `kukuri-tauri/src/tests/unit/scenario/userSearchPaginationArtefact.test.tsx` を新設し、`sanitizeUserSearchQuery` / `detectUserSearchHelper`（`useUserSearchQuery.ts` からエクスポート）と `SearchErrorState` を組み合わせて「2文字未満入力→補助検索→レート制限 UI」フローを JSON (`test-results/user-search-pagination/search-error/<timestamp>-search-error-state.json`) に保存する仕組みを追加。
- [x] `scripts/test-docker.{sh,ps1}` の `user-search-pagination` シナリオを拡張し、新しい Vitest ターゲットと `USER_SEARCH_SCENARIO_TIMESTAMP` を渡して artefact を生成。`nightly.yml` では `user-search-pagination-search-error` アップロード処理を追加し、`test-results/user-search-pagination/{logs,reports,search-error}` を連携。
- [x] `docs/03_implementation/p2p_mainline_runbook.md` 6.4、`phase5_ci_path_audit.md`、`phase5_user_flow_summary.md`、`docs/01_project/progressReports/nightly.partial-feature-usage.md` を更新し、短文補助検索と SearchErrorState artefact の再現手順・artefact パスを記録。`docs/01_project/roadmap.md` では当該バックログを完了扱いへ更新。
- [x] `./scripts/test-docker.sh ts --scenario user-search-pagination` をビルド込みで実行し、`tmp/logs/user_search_pagination_20251120-002747.log`・`test-results/user-search-pagination/reports/20251120-002747-*.json`・`test-results/user-search-pagination/search-error/20251120-002747-search-error-state.json` を取得済み。CI への反映確認として `user-search-pagination-logs`／`...-reports`／`...-search-error` artefact を Nightly Runbook に追記。
...
- `kukuri-tauri/src/hooks/useUserSearchQuery.ts`
- `kukuri-tauri/src/tests/unit/scenario/userSearchPaginationArtefact.test.tsx`
...
- [x] Docker Test Suite  `-D warnings` Œ~Ă߁AgpW[ (`memory_cache` Ȃ) ƃv[e[Vw̖znh/DTO 𐮗AeXgp\bh `#[cfg(test)]` ɈڂĖ{ԃrh珜OB
- [x] vtB[Ao^[L`Pbg kind:30078 ^f[^ADHT/Gossip/Distributor ԂȂǕێĂ邾tB[hɎpiof[VEOE폜jǉ Rust  dead_code B
- [x] `ApiResponse`E`AuthHandler`E`state.rs` Ȃ AppState ӂVACI  Docker Wuł clean build ɂȂ邱Ƃ `gh act` ŊmFB
...
- `kukuri-tauri/src-tauri/src/state.rs` Ȃ dead_code CōXV Rust t@Cꎮ
...
## Direct Message ナイトリー/会話ページング整備

- [x] Rust: `list_direct_message_conversations` / DTO / `TauriApi` / `DirectMessageService` をカーソル + `has_more` 対応に拡張し、`DirectMessageConversationCursor` で `bucket:last_message_created_at:base64(npub)` をシリアライズ。SQLite リポジトリは `QueryBuilder` で条件句を構築し、`DirectMessageConversationPageRaw` を返却するよう更新。
- [x] フロント: `DirectMessageInbox` を `useInfiniteQuery(['direct-message-conversations'])` + TanStack Virtualizer へ移行し、検索・Enter 補完・既読バッジに加えて `Load more` ボタンとローディング行を実装。`useDirectMessageBootstrap` は 50 件単位でページを巡回し、ストアの既読/未読をハイドレートする。
- [x] `scripts/test-docker.{sh,ps1}` の usage に `--scenario direct-message` を明記し、`.github/workflows/nightly.yml` に `direct-message` ジョブ（`nightly.direct-message-{logs,reports}` artefact）を追加。Runbook Chapter5 / `phase5_user_flow_inventory.md` / `phase5_ci_path_audit.md` / `nightly.partial-feature-usage.md` へ Nightly 手順とログ/JSON パスを追記。
- [x] Vitest: `DirectMessageInbox.test.tsx` を QueryClientProvider でラップし、`listDirectMessageConversations` モックが `nextCursor` / `hasMore` を扱うよう更新。`DirectMessageDialog.test.tsx` / `Header.test.tsx` も新レスポンスに追従。Rust 側は `tests/contract/direct_messages.rs` を新しいメソッドシグネチャへ合わせた。
...
- `scripts/test-docker.sh`
- `scripts/test-docker.ps1`
- `kukuri-tauri/src-tauri/src/application/ports/repositories.rs`
- `kukuri-tauri/src-tauri/src/application/services/direct_message_service.rs`
- `kukuri-tauri/src-tauri/src/application/services/direct_message_service/tests.rs`
- `kukuri-tauri/src-tauri/src/infrastructure/database/sqlite_repository/direct_messages.rs`
- `kukuri-tauri/src-tauri/src/presentation/dto/direct_message_dto.rs`
- `kukuri-tauri/src/lib/api/tauri.ts`
- `kukuri-tauri/src/hooks/useDirectMessageBootstrap.ts`
- `kukuri-tauri/src/components/directMessages/DirectMessageInbox.tsx`
- `kukuri-tauri/src/tests/unit/components/directMessages/DirectMessageInbox.test.tsx`
- `kukuri-tauri/src/tests/unit/components/directMessages/DirectMessageDialog.test.tsx`
...
- `./scripts/test-docker.ps1 ts -Scenario direct-message -NoBuild`
