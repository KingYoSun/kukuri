...
## Community Nodes / `cn-relay` `/healthz` `/metrics` 契約 + 必須メトリクス互換固定

- [x] `cn-relay` に `/healthz` の契約テストを追加し、`200 {"status":"ok"}` と `503 {"status":"unavailable"}` の shape 互換を固定
- [x] `cn-relay` に `/metrics` の契約テストを追加し、Prometheus content-type 互換（`text/plain; version=0.0.4`）を固定
- [x] Runbook 必須メトリクス（`ws_connections` / `ws_req_total` / `ws_event_total` / `ingest_received_total` / `ingest_rejected_total` / `gossip_received_total` / `gossip_sent_total` / `dedupe_hits_total` / `dedupe_misses_total`）の公開互換をテストで固定
- [x] `community_nodes_roadmap.md` の該当未実装項目を完了に更新
- [x] 進捗レポートを `docs/01_project/progressReports/2026-02-11_cn_relay_healthz_metrics_contract.md` として作成

## 検証（`cn-relay` 契約 + メトリクス）

- [x] `./scripts/test-docker.ps1 rust`（成功）
  - ログ: `tmp/logs/test-docker-rust-cn-relay-metrics-20260211-191220.log`
- [x] `gh act --workflows .github/workflows/test.yml --job format-check`（初回/再実行で失敗 → `cargo fmt --all` 適用後に成功）
  - 失敗ログ: `tmp/logs/gh-act-format-check-cn-relay-metrics-20260211-191324.log`
  - 失敗ログ: `tmp/logs/gh-act-format-check-cn-relay-metrics-20260211-191536.log`
  - 成功ログ: `tmp/logs/gh-act-format-check-cn-relay-metrics-20260211-191756.log`
- [x] `gh act --workflows .github/workflows/test.yml --job native-test-linux`（成功）
...
## Community Nodes / `cn-user-api` 認証・同意・課金メータの回帰テスト追加

- [x] `cn-user-api` 契約テストに回帰ケース `auth_consent_quota_metrics_regression_counters_increment` を追加
- [x] API 実行で `auth_success_total` / `auth_failure_total` / `consent_required_total` / `quota_exceeded_total{metric="max_topics"}` の増分を検証
- [x] `community_nodes_roadmap.md` の該当未実装項目を完了に更新
- [x] 進捗レポートを `docs/01_project/progressReports/2026-02-11_cn_user_api_auth_consent_quota_metrics_regression.md` として作成

## 検証（`cn-user-api` メータ回帰テスト）

- [x] `./scripts/test-docker.ps1 rust`（成功）
- [x] `docker build -t kukuri-postgres-age ./kukuri-community-node/docker/postgres-age`（成功）
- [x] `docker run --rm -v "${PWD}:/app" -w /app/kukuri-community-node -e DATABASE_URL=postgres://cn:cn_password@host.docker.internal:15432/cn rust:1.88-bookworm /usr/local/cargo/bin/cargo test -p cn-user-api auth_consent_quota_metrics_regression_counters_increment -- --nocapture --test-threads=1`（成功）
...
## Community Nodes / Ops Runbook バックアップ・リストア運用スクリプト化（復旧ドリル）

- [x] `scripts/test-docker.ps1` に `recovery-drill` コマンドを追加し、`pg_dump` 世代管理（`COMMUNITY_NODE_BACKUP_GENERATIONS`）を実装
- [x] `pg_restore` 復旧手順（`dropdb --force` -> `createdb` -> `pg_restore`）を自動化
- [x] 復旧ドリル（seed -> backup -> 障害模擬(TRUNCATE) -> restore -> 件数検証）を実装し、ログ/サマリを出力
- [x] `docs/03_implementation/community_nodes/ops_runbook.md` に運用コマンド/出力先/ドリル内容を反映
- [x] `community_nodes_roadmap.md` の該当未実装項目を完了に更新
- [x] 進捗レポートを `docs/01_project/progressReports/2026-02-11_community_node_backup_restore_recovery_drill.md` として作成

## 検証（復旧ドリル）

- [x] `./scripts/test-docker.ps1 recovery-drill -NoBuild`（成功）
