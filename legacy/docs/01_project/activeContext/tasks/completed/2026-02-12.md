...
## Community Nodes / `cn-bootstrap` 更新ヒント publish 経路（gossip/DHT ヒント併用）

- [x] `cn-bootstrap` に 39000/39001 更新時の hint publish 経路を追加（`pg_notify('cn_bootstrap_hint', payload)`）
- [x] publish payload に `refresh_paths` と変更概要（descriptor/topic_service 変更・削除）を追加し、通知受信→HTTP再取得の整合フローを実装
- [x] `cn-core` メトリクスへ `bootstrap_hint_publish_total{service,channel,result}` を追加し、`result=success|failure` の両方を記録
- [x] `cn-bootstrap` 統合テストを追加（通知受信→HTTP再取得、publish成功/失敗メトリクス）
- [x] `docs/03_implementation/community_nodes/services_bootstrap.md` に実装メモ（hint publish 経路とメトリクス）を追記
- [x] `community_nodes_roadmap.md` の該当未実装項目を完了（`[x]`）へ更新
- [x] 進捗レポート `docs/01_project/progressReports/2026-02-12_cn_bootstrap_hint_publish_path.md` を作成

## 検証（`cn-bootstrap` 更新ヒント publish）

- [x] `cargo fmt --all`（`kukuri-community-node`）
- [x] `./scripts/test-docker.ps1 rust`（成功）
- [x] `docker compose -f docker-compose.test.yml run --rm -e DATABASE_URL=postgres://cn:cn_password@localhost:5432/cn test-runner bash -lc "source /usr/local/cargo/env && cd /app/kukuri-community-node && cargo test --workspace --all-features && cargo build --release -p cn-cli"`（初回失敗: DB未起動で `PoolTimedOut`、再実行成功）
- [x] `docker compose -f docker-compose.test.yml run --rm -v ${PWD}:/workspace -v kukuri-cargo-registry:/usr/local/cargo/registry -v kukuri-cargo-git:/usr/local/cargo/git -v kukuri-cargo-target:/workspace/kukuri-community-node/target -w /workspace/kukuri-community-node -e DATABASE_URL=postgres://cn:cn_password@localhost:5432/cn test-runner bash -lc "source /usr/local/cargo/env && cargo test --workspace --all-features && cargo build --release -p cn-cli"`（初回失敗: 既存DB残骸で `events_pkey` 重複、DB再初期化後に成功）
- [x] `gh act --workflows .github/workflows/test.yml --job format-check`（成功）
  - `tmp/logs/gh-act-format-check-cn-bootstrap-hint-20260213-042823.log`
- [x] `gh act --workflows .github/workflows/test.yml --job native-test-linux`（成功）
  - `tmp/logs/gh-act-native-test-linux-cn-bootstrap-hint-20260213-042945.log`
- [x] `gh act --workflows .github/workflows/test.yml --job community-node-tests`（成功）
  - `tmp/logs/gh-act-community-node-tests-cn-bootstrap-hint-20260213-043613.log`
