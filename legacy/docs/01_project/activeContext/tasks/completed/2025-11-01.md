...
- [x] `cargo clippy --workspace --all-features -- -D warnings` をゼロエラーで完走させる（2025年10月31日: `kukuri-tauri/src-tauri` と `cn-cli` の両方で実行し警告ゼロを確認）
...
- [x] `cargo test`（Docker 経由で kukuri-tauri、ローカルで cn-cli）および `pnpm test` を実行しリグレッションなしを確認
  - 2025年11月01日: `./scripts/test-docker.ps1 rust` で Rust テストを完走（ログで成功確認）、`cn-cli` はローカル `cargo test`、フロントは `pnpm test` を実施。
