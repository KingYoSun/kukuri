...
- [x] `cd kukuri-tauri/src-tauri && cargo test` および `cd kukuri-community-node && cargo test` で Rust 側回帰が無いことを検証。
...
## Stage4: Offline topic-create queue / pendingTopics UI 完了

- [x] Rust: `TopicService::enqueue_topic_creation` / `list_pending_topics` / `mark_pending_topic_synced|failed` を追加し、`topics_pending` テーブル＋`PendingTopicRepository` 経由で `OfflineActionType::CREATE_TOPIC` を永続化。`tests/integration/topic_create_join.rs` を実装して pending → synced/failed → `P2PService::join_topic` までの再送フローを自動化。
- [x] フロント: `topicStore.pendingTopics` / `queueTopicCreation` / `refreshPendingTopics` を実装し、`TopicSelector` の「保留中のトピック」表示とステータスバッジ、`TopicFormModal` のオフライン経路（`watchPendingTopic` → `resolvePendingTopic` → `applyTopicAndResume`）を追加。Radix Slot の ref 警告を解消するため `Input` を `forwardRef` 化。
- [x] QA: `npx pnpm vitest run` で TopicSelector/PostComposer/Sidebar/`topicCreateOffline` シナリオをまとめて実行し、`../tmp/logs/topic_create_host_20251112-231141.log` を採取。Docker では `./scripts/test-docker.ps1 ts -Scenario topic-create` を実行し、`tmp/logs/topic_create_20251112-231334.log` と `test-results/topic-create/20251112-231334-*.json`（4件）を生成し Nightly artefact (`topic-create-logs` / `topic-create-reports`) に登録。
- [x] Docs/Runbook: `phase5_user_flow_inventory.md` 5.9、`phase5_user_flow_summary.md`、`phase5_ci_path_audit.md`、`docs/03_implementation/p2p_mainline_runbook.md` 5章、`tauri_app_implementation_plan.md` の Phase5 行を Stage4 完了内容（ログ採取パス/テスト ID）で更新。`tasks/status/in_progress.md` から当該タスクを削除。
- [x] Rust テスト: Windows ネイティブ `cargo test`（`kukuri-tauri/src-tauri`）は既知の `STATUS_ENTRYPOINT_NOT_FOUND (0xc0000139)` で停止するため、同ログを記録しつつ `./scripts/test-docker.ps1 rust -NoBuild` を実行して全テストを完走。`cd kukuri-community-node && cargo test` はネイティブで成功。
...
- `./scripts/test-docker.ps1 rust -NoBuild`
- `cd kukuri-community-node && cargo test`
- `cd kukuri-tauri && powershell -Command "$repoRoot = Split-Path -Parent (Get-Location); $log = Join-Path $repoRoot 'tmp/logs/topic_create_host_20251112-231141.log'; npx pnpm vitest run src/tests/unit/components/topics/TopicSelector.test.tsx src/tests/unit/components/posts/PostComposer.test.tsx src/tests/unit/components/layout/Sidebar.test.tsx src/tests/unit/scenarios/topicCreateOffline.test.tsx 2>&1 | Tee-Object -FilePath $log"`
