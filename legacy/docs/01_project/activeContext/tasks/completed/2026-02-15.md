...
## Community Nodes 優先タスク整理完了（2026年02月15日）

- [x] `docs/01_project/activeContext/tasks/priority/community_nodes*.md`（8ファイル）を完了済みとして整理し、先頭メタ情報に `ステータス: 完了` を追記。
- [x] 同ファイル群は参照性維持のため `priority` 配下に残し、運用上は「新規着手対象外（履歴参照用）」であることを明記。
- [x] 新規タスクの起点を `docs/01_project/activeContext/tasks/priority/critical.md` に統一。
...
## Issue #22 Task4 完了（node-level 上限回帰テスト追加、2026年02月15日）

- 対象: node-level 同時取込 topic 数上限の回帰テスト補完（`cn-admin-api` 契約 + `cn-relay` 統合）
- 実施内容:
  - `cn-admin-api` 契約テストに、`current > limit`（既に上限超過）時でも `POST /v1/admin/subscription-requests/{request_id}/approve` が `429 + NODE_SUBSCRIPTION_TOPIC_LIMIT_REACHED` を返す回帰を追加。
  - 併せて `details.metric/scope/current/limit` と、拒否時に request が `pending` 維持・`topic_subscriptions`/`node_subscriptions` に副作用が発生しない契約を固定。
  - `cn-relay` 統合テストに、enabled topic が上限を超えて増えても `load_enabled_topics(..., max_concurrent_topics)` の選択件数が増加しない（件数は上限で固定）回帰を追加。
- 検証:
  - `docker compose -f docker-compose.test.yml up -d community-node-postgres community-node-meilisearch`（成功）
  - `DOCKER_CONFIG=/tmp/docker-config docker compose -f docker-compose.test.yml build test-runner`（成功）
  - `docker run --rm --network kukuri_community-node-network -e DATABASE_URL=postgres://cn:cn_password@community-node-postgres:5432/cn -e MEILI_URL=http://community-node-meilisearch:7700 -e MEILI_MASTER_KEY=change-me -e RUST_TEST_THREADS=1 -v "$(git rev-parse --show-toplevel):/workspace" -w /workspace/kukuri-community-node kukuri-test-runner bash -lc "set -euo pipefail; source /usr/local/cargo/env; cargo test -p cn-admin-api subscription_request_approve_rejects_when_node_topic_limit_ -- --nocapture --test-threads=1; cargo test -p cn-relay node_subscription_limit_prevents_desired_topic_growth_when_over_limit -- --nocapture --test-threads=1"`（成功, 3 tests passed）
  - `XDG_CACHE_HOME=/tmp/xdg-cache NPM_CONFIG_PREFIX=/tmp/npm-global DOCKER_CONFIG=/tmp/docker-config gh act --workflows .github/workflows/test.yml --job format-check`（成功, ログ: `tmp/logs/gh-act-format-check-issue22-node-level-limit-regression-tests.log`）
  - `XDG_CACHE_HOME=/tmp/xdg-cache NPM_CONFIG_PREFIX=/tmp/npm-global DOCKER_CONFIG=/tmp/docker-config gh act --workflows .github/workflows/test.yml --job native-test-linux`（成功, ログ: `tmp/logs/gh-act-native-test-linux-issue22-node-level-limit-regression-tests.log`）
  - `XDG_CACHE_HOME=/tmp/xdg-cache NPM_CONFIG_PREFIX=/tmp/npm-global DOCKER_CONFIG=/tmp/docker-config gh act --workflows .github/workflows/test.yml --job community-node-tests`（失敗）
    - 1回目: `transactional_admin_mutations_rollback_when_audit_log_write_fails` が `left: 200, right: 500` で失敗
    - 2回目（再実行）: `trigger "test_audit_failures_trigger" already exists` で失敗
...
- 進捗レポート: `docs/01_project/progressReports/2026-02-15_issue22_node_level_limit_regression_tests.md`

## Issue #22 Task4 フォローアップ完了（PR #26 Community Node Tests fix loop、2026年02月15日）

- 対象: PR #26 の `Community Node Tests` 失敗ループ（Actions job `63674268397`）を安定化し、同 PR スコープで deterministic に通過させる。
- 根本原因:
  - `cn-core` の `health.rs` テストが `unsafe std::env::set_var/remove_var` を使っており、並列テスト時の環境変数競合で SIGSEGV（signal 11）を誘発。
  - `cn-admin-api` 契約テストは test trigger 作成が並列 DDL 競合を起こし、`trigger already exists` / `tuple concurrently updated` が発生。
  - PR #26 追加テスト `subscription_request_approve_rejects_when_node_topic_limit_already_exceeded` は共有状態前提の同値比較（`current`/`limit`）が並列実行で揺れ、偽陽性失敗を起こす。
- 実施内容:
  - `cn-core/src/health.rs`: 環境変数依存ロジックを `parse_health_targets_with` へ抽出し、テストを in-memory map ベースへ変更（unsafe env mutation を除去）。
  - `cn-admin-api/src/contract_tests.rs`: audit/commit failure trigger の準備処理を `pg_advisory_xact_lock` で直列化し、DDL 競合を回避。
  - 同テストの over-limit 検証を `current > limit` 契約中心にし、共有状態の揺れに耐えるアサーションへ調整。
- 検証:
  - Community Node コンテナ経路: `cargo test -p cn-core --all-features`（成功）
  - Community Node コンテナ経路: `cargo test -p cn-admin-api --all-features`（成功, 43 passed）
  - Community Node CI 同等経路: `cargo test --workspace --all-features; cargo build --release -p cn-cli`（成功）
  - `gh act --workflows .github/workflows/test.yml --job format-check`（成功, `tmp/logs/gh-act-format-check-issue22-pr26-fix-loop.log`）
  - `gh act --workflows .github/workflows/test.yml --job native-test-linux`（成功, `tmp/logs/gh-act-native-test-linux-issue22-pr26-fix-loop.log`）
  - `gh act --workflows .github/workflows/test.yml --job community-node-tests`（成功, `tmp/logs/gh-act-community-node-tests-issue22-pr26-fix-loop.log`）
- 進捗レポート: `docs/01_project/progressReports/2026-02-15_issue22_pr26_community_node_tests_fix_loop.md`

## Issue #22 最終再監査完了（2026年02月15日）

- 対象: `docs/03_implementation/community_nodes` と `docs/01_project/activeContext/tasks/priority/community_nodes_roadmap.md`
- 監査基準: strict gate は Issue #5 fallback（`https://github.com/KingYoSun/kukuri/issues/5#issuecomment-3900483686`）を適用。
- 結果:
  - Gate1-6 すべて PASS（`GET /v1/bootstrap/hints/latest` 401/428/429 境界、`spawn_bootstrap_hint_listener` 実DB受信テスト、`CommunityNodePanel`/`PostSearchResults` テスト、Issue #22 DoS 4タスク完了、追加不足タスクなし）。
  - `community_nodes_roadmap.md` の未完3項目を完了化し、Issue #22 の追加実装タスクは 0 件を確認。
  - Issue #22 はクローズ可能。
- 進捗レポート: `docs/01_project/progressReports/2026-02-15_issue22_final_reaudit.md`
