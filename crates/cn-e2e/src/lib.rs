...
/// wiremock の Arachnid 模擬が要求する資格情報（実物ではない合成値）。
const ARACHNID_USERNAME: &str = "e2e-shield-user";
const ARACHNID_PASSWORD: &str = "e2e-shield-pass";
...
async fn mount_default_allow_mocks(arachnid: &MockServer, vlm: &MockServer) {
    Mock::given(method("POST"))
        .and(path("/v1/media"))
        .and(basic_auth(ARACHNID_USERNAME, ARACHNID_PASSWORD))
        .respond_with(
...
fn build_providers(
    ...
    let arachnid = ProjectArachnidShieldProvider::with_credentials(
        ...
        ShieldCredentials::new(ARACHNID_USERNAME, ARACHNID_PASSWORD),
    )
...
impl E2eStack {
    ...
    pub async fn boot(prefix: &str) -> Result<Option<Self>> {
        ...
        mount_default_allow_mocks(&arachnid, &vlm).await;
        ...
        let media_fetcher: Arc<dyn MediaFetcher> = Arc::new(
            BlobMediaFetcher::new(indexer_blob_service, MediaFetchConfig::default())
                .with_metrics(Arc::clone(&runtime_state)),
        );
        let providers = build_providers(&arachnid.uri(), &vlm.uri(), media_fetcher)?;
