...
async fn p2p_info(State(state): State<AppState>) -> ApiResult<Json<RelayP2pInfoResponse>> {
...
async fn handle_socket(state: AppState, _addr: SocketAddr, socket: WebSocket) {
    ...
    loop {
        tokio::select! {
            _ = tick.tick() => {
                if let Ok(rollout) = load_auth_rollout(&state.pool, RELAY_SERVICE_NAME).await
                    && let Err(error) = enforce_auth_timeout(&state, &mut sender, &mut session, &rollout).await
                {
