...
# HA managed Postgres（REGIONAL / Multi-AZ）。private IP で VM から接続する。
module "cloudsql" {
...
# blob/media 用 object storage（canonical DB には混ぜない rebuildable cache）。
module "blob_storage" {
  source = "../../modules/gcp-blob-storage"

  enabled               = var.blob_storage_enabled
  name_prefix           = var.name_prefix
  project_id            = var.project_id
  location              = var.region
  cache_ttl_days        = var.blob_cache_ttl_days
