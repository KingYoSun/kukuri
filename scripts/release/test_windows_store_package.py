...
class WindowsStorePackageContracts(unittest.TestCase):
    ...
    def test_packaging_keeps_store_and_local_test_signatures_separate(self):
        ...
        self.assertIn("Import-PfxCertificate", source)
        ...
        self.assertIn("certutil.exe -user -delstore My $certificate.Thumbprint", source)
        self.assertIn("Import-Certificate -FilePath $certificateOutputPath -CertStoreLocation Cert:\\CurrentUser\\TrustedPeople", source)
        self.assertIn("certutil.exe -user -delstore TrustedPeople $certificate.Thumbprint", source)
        self.assertIn("Failed to remove temporary local-test certificates", source)
